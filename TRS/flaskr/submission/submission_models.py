...
class Submission(db.Model):
  ...
  student_id = Column(UUID(as_uuid=True), ForeignKey('students.id'), unique=False, nullable=False)
  assignment_id = Column(UUID(as_uuid=True), ForeignKey('assignments.id'), unique=False, nullable=False)
  assignment = relationship("Assignment", back_populates="submission", uselist=False)

  # array of multiple testcase with pass/fail = 1/0
  # each number of testcase is differenct from assignments
  scores = Column(ARRAY(db.Boolean), unique=False, nullable=True)
