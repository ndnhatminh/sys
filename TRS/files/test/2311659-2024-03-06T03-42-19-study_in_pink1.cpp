...
int ceil_(double x) {
	...
	if (x - static_cast<double>(y) >= 0.01) {
		return y + 1;
	}
	return y;
...
int scale(int x, double f) {
	if (f > 0.0) {
		return x + ceil_(static_cast<double>(x) * f);
	} else {
		return x - static_cast<int>(static_cast<double>(x) * -f);
	}
}
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
	...
	double P2 = 0.0;
	{
		...
		for (;;) {
			...
			if ( (half_m1 > M1 && e2_odd) || M1 <= 0 ) break;
...
int checkPassword(const char * s, const char * email) {
	...
	for (const char* c = s+1; *c; c++) {
		...
		if (consecutive > 2) {
			return -400 - ((int)(c - s) - consecutive + 1);
		}
	}
