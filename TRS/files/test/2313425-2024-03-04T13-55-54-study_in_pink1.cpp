...
// Task 5
int XuatHien(const char* arr_pwds[], size_t num_pwds, const char* password) {                                                                                                                                                                                                   
    int count = 0;
...
int findCorrectPassword(const char* arr_pwds[], int num_pwds) {
    ...
    int maxLength = 0;
    int maxCount = 0;
    const char* Current_Pwds;
    const char* Correct_Pwds = nullptr;

    for (size_t i = 0; i < num_pwds; ++i) {
        Current_Pwds = arr_pwds[i];
        int SoLanXuatHien = XuatHien(arr_pwds, num_pwds, Current_Pwds);
        if (SoLanXuatHien > maxCount || (SoLanXuatHien == maxCount && strlen(Current_Pwds) > maxLength)) {
            maxCount = SoLanXuatHien;
            maxLength = strlen(Current_Pwds);
