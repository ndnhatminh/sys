...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
	...
	while (E2 % 2 == 0) //E2 chan
	{
		...
		if (hp1F < 200)
		{
			hp1F = check(ceil(hp1F * 1.3), hpFirstpoint, hpLastpoint);
			m1F = m1F - 150;
			spending = spending + 150;
		}
		else
		{
			hp1F = check(ceil(hp1F * 1.1), hpFirstpoint, hpLastpoint);
			m1F = m1F - 70;
		...
		exp1F = check(ceil(exp1F * 0.9), expFirstpoint, expLastpoint);
		flag = false;
		break;
	}
	...
	{
		finalP = 1;
		exp1F = check(ceil(exp1F * 0.75), expFirstpoint, expLastpoint);
	}
	else
	{
	...
	if (finalP < 0.5)
	{
		hp1F = check(ceil(hp1F * 0.85), hpFirstpoint, hpLastpoint);
		exp1F = check(ceil(exp1F * 1.15), expFirstpoint, expLastpoint);
	}
	else if (finalP >= 0.5 && finalP < 1)
	{
		hp1F = check(ceil(hp1F * 0.9), hpFirstpoint, hpLastpoint);
		exp1F = check(ceil(exp1F * 1.2), expFirstpoint, expLastpoint);
	}
	EXP1 = exp1F;
	EXP1 = check(EXP1, expFirstpoint, expLastpoint);
	HP1 = hp1F;
