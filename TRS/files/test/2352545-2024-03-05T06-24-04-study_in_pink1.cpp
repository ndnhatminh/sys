...
{  EXP1-=E1;
   ...
   else if(E1<=79)  { EXP2 += 19+ceil(E1/4.0);getEXP(EXP1);getEXP(EXP2);
   if (EXP2>200) EXP2+=21+ceil(E1/9.0);}
                     ...
                     getEXP(EXP1);
                     getEXP(EXP2);
                     if(EXP2>400) {EXP2+=17+ceil(E1/16.0);
                     getEXP(EXP1);
                     getEXP(EXP2);
...
{int M=M1;
 ...
 else {if (M1!=0) {traceLuggageRoad2_Event1(HP1,M1);
                   if (M1!=0) {traceLuggageRoad2_Event2(EXP1,M1);
                               if (M1!=0) {traceLuggageRoad2_Event3(EXP1,M1);}
                              }
                  }
      } 
...
{EXP1=ceil(EXP1*115.0/100);getEXP(EXP1);
 HP1=ceil(HP1*85.0/100);
}
// P>=50%
void incident_3(int &EXP1,int &HP1)
{EXP1=ceil(EXP1*12.0/10);getEXP(EXP1);
...
P=(P1+P2+P3)/3;
if (P==1) incident_1(EXP1);
else if (P<0.5) incident_2(EXP1,HP1);
else if (P>=0.5) incident_3(EXP1,HP1);
    return HP1 + EXP1 + M1;
}
// Task 3
// Function to calculate the absolute sum of digits
int absSumOfDigits(int num) {
    int sum = 0;
    while (num != 0) {
        sum += num % 10;
        num /= 10;
    ...
    if (sum >= 10) {
        return absSumOfDigits(sum);
    }
    return sum;
}

// Function to calculate chaseTaxi score
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3) {
    // Initialize matrix for taxi with size 10x10
    int arr[10][10] = {0};

    // Fill the matrix with scores and find bigger and smaller values
    int bigger = -999, smaller = 999;
    int posI, posJ;
    for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 10; ++j) {
    ...
    // Find meeting point
    int sumBigger = absSumOfDigits(bigger);
    int sumSmaller = absSumOfDigits(smaller);
    while (sumBigger >= 10 || sumSmaller >= 10) {
        sumBigger = absSumOfDigits(sumBigger);
        sumSmaller = absSumOfDigits(sumSmaller);
    }
    posI = sumBigger;
    posJ = sumSmaller;

    // Find max score on two diagonal lines through meeting point
    int sherlockPoint = arr[posI][posJ];
    for (int i = 0; i < 10; ++i) {
        int j1 = posJ - posI + i;
        int j2 = posI + posJ - i;
        if (j1 >= 0 && j1 < 10) {
            sherlockPoint = max(sherlockPoint, arr[i][j1]);
        }
        if (j2 >= 0 && j2 < 10) {
            sherlockPoint = max(sherlockPoint, arr[i][j2]);
        }
    }

    // Compare scores
    int taxiPoint = abs(arr[posI][posJ]);
    sherlockPoint = abs(sherlockPoint);
    if (taxiPoint > sherlockPoint * 2) {
        return taxiPoint;
    } else {
        if (taxiPoint > 2 * sherlockPoint) {
            HP1 += ceil(0.12 * HP1);
            HP2 += ceil(0.12 * HP2);
            EXP1 += ceil(0.10 * EXP1);
            EXP2 += ceil(0.10 * EXP2);
