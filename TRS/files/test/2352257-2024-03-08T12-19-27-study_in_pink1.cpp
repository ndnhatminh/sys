...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3)
{
    ...
    //Vi tri
     ...
     for (int i = 0; i < 10; i=i+1) {
        for (int j = 0; j < 10; j=j+1) {
           if (map[i][j] > (E3*2)) h=h+1 ; 
           if (map[i][j]<-E3) k=k+1;
        }
    }

    while (h>9) {h= h/10 + h%10;}
    while (k>9) {k= k/10 + k%10;}/*VI tri map[h][k]*/
    TS=map[h][k];
    ...
    if (abs(TS)<=DS) {EXP1=ceil(EXP1*1.00+EXP1*0.12);
                     HP1=ceil(HP1*1.0+0.1*HP1);
                     EXP2=ceil(EXP2*1.00+0.12*EXP2);
                     HP2=ceil(HP2*1.0+0.1*HP2);   
...
int checkPassword(const string & s, const string & email) {
    ...
    for (int i=0; i< dai; i=i+1){
        if (s[i] == '@' || s[i] ==  '#' || s[i] == '%' || s[i] ==  '$' || s[i] ==  '!') lan=lan+1;
