...
int firstMeet(int & exp1, int & exp2, int e1) {
    ...
    if ((e1>=0)&&(e1<=3))
    {
        ...
        if (D%2==0)
        {
            exp1=ceil(exp1+(float)D/200-1e-7);
            exp1=CheckEXP(exp1);
        }
        else
        {
            exp1=ceil(exp1-(float)D/100-1e-7);
            exp1=CheckEXP(exp1);
        }
    }
    else if ((e1>=4)&&(e1<=99))
    {
        if ((e1<=19)&&(e1>=4))
        {
            exp2=ceil((float)exp2 + (float)e1/4 + 19-1e-7);
            exp2=CheckEXP(exp2);
        }
        else if ((e1>=20)&&(e1<=49))
        {
            exp2=ceil((exp2 +(float) e1/9 + 21) -1e-7);
            exp2=CheckEXP(exp2);
        }
        else if ((e1>=50)&&(e1<=65))
        {
            exp2=ceil((float)exp2 + (float)e1/16 + 17 -1e-7);
            exp2=CheckEXP(exp2);
        }
        else if ((e1>=66)&&(e1<=79))
        {
            ...
            if (exp2>200)
            {
                exp2=ceil((float)exp2 +(float) e1/9 + 21-1e-7 );
                exp2=CheckEXP(exp2);
            }
        }
        else if ((e1>=80)&&(e1<=99))
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if (E2 % 2 == 0)
  {
    ...
    HP1 = CheckHP (ceil ((float) HP1 * (0.83-1e-7)));
    EXP1 = CheckEXP (ceil ((float) EXP1 * (1.17-1e-7)));
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    if (absolute(road[meeti(road,E3)][meetj(road,E3)])> homwas[meeti(road,E3)][meetj(road,E3)])
    {
        ...
        EXP1=CheckEXP(ceil((float)EXP1*(0.88-1e-7)));
        EXP2=CheckEXP(ceil((float)EXP2*(0.88-1e-7)));
        HP1=CheckHP(ceil((float)HP1*(0.9-1e-7)));
        HP2=CheckHP(ceil((float)HP2*(0.9-1e-7)));
    }
    else
    {
        point = homwas[meeti(road,E3)][meetj(road,E3)];
        EXP1=CheckEXP(ceil((float)EXP1*(1.12-1e-7)));
        EXP2=CheckEXP(ceil((float)EXP2*(1.12-1e-7)));
        HP1=CheckHP(ceil((float)HP1*(1.1-1e-7)));
        HP2=CheckHP(ceil((float)HP2*(1.1-1e-7)));
    }
    //-1
    return point;
}
...
int checkPassword(const char * s, const char * email) {
    ...
    if(result==0){result=checkse(s1,se);}
    if(result==0){result=checksam(s);}
    if(result==0){result=checkspe(s);}
    if(result==0){result=checkremain(s);}
