...
int checkPassword(const char * s, const char * email) {
    ...
    bool TooShort = (Size < 8);
    bool TooLong = (Size > 20);
    ...
    bool containsConsecutiveChars = false;
    ...
    } else if (containsConsecutiveChars) {
        return -(400 + static_cast<int>(strstr(s, se) - s));
    } else if (containsInvalidChar) {
        return static_cast<int>(strchr(s, '@') - s);  
    } else {
        return -10;
    }
}
