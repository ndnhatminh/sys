...
int checkPassword(const char * s, const char * email) {
    ...
    const char* locate = strchr (email, '@'); 
    int length = locate - email;
     // Set special digits
    const char* special = "!@#$%";
    /// Copy the string founded to se
    strncpy(se, email, length);
    se[length] = '\0'; // to sure about the end of the se
    ...
    // nullptr meaning for False of a pointer value
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    ...
    int maxLength = 0; // max length of the one that maybe the correct one
    int maxFreq = 0; // max frequency of the one that maybe the correct one
    int correctPass = 0; // first position of correct one
    while (counter < num_pwds) // loop to check each one
    {
        if (freq[counter] > maxFreq)// the one has largest frequency will be the correct password.
        { 
            maxFreq = freq[counter];
            maxLength = length[counter];
            correctPass = counter;
        }
        else if (freq[counter] == maxFreq && length[counter] > maxLength)
