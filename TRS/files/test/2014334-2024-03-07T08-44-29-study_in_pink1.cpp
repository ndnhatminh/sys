...
int findFirstIndex(const char* s, char target) {
    for (int i = 0; s[i] != '\0'; ++i) {
        if (s[i] == target) {
            return i;
        }
...
int checkPassword(const char * s, const char * email) {
    ...
    int atIndex = findFirstIndex(email, '@');
    if (atIndex == -1 || atIndex > 100) return -3; // Email không hợp lệ

    // Tạo chuỗi "se" từ email
    char se[101];
    // strncpy(se, email, atIndex);
    int i;
    for (i = 0; i < atIndex && email[i] != '\0'; ++i) {
        se[i] = email[i];
    }
    se[atIndex] = '\0';

    // Kiểm tra có chứa chuỗi "se" không
    int pos = findFirstIndex(s, se[0]);
    while (pos != -1 && pos + atIndex <= length) {
        int i;
        for (i = 0; i < atIndex; ++i) {
            if (s[pos + i] != se[i]) {
                break; // Kết thúc vòng lặp nếu có ký tự khác nhau
            }
        }
        if (i == atIndex) {
            return -(300 + pos); // Mật khẩu chứa chuỗi "se"
        }
        pos = findFirstIndex(s + pos + 1, se[0]); // Dịch chuyển vị trí bắt đầu của chuỗi s
    }
