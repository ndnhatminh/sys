...
{
    // Tính số chính phương gần EXP1 nhất S:
    // Nếu EXP1 là số chính phương thì S = EXP1
    // Số chính phương gần EXP1 thứ nhất
    double a = static_cast<int>(my_sqrt(EXP)) * static_cast<int>(my_sqrt(EXP));
    // Số chính phương gần EXP1 thứ hai
    double b = (static_cast<int>(my_sqrt(EXP)) + 1) * (static_cast<int>(my_sqrt(EXP)) + 1);

    int S;
    if ((EXP - a) <= (b - EXP)) // nếu khoảng cách giữa a với EXP1 gần hơn so với của b thì
    {
        S = a; // a là số chính phương gần EXP1 nhất
    }
    else // nếu không thì
    {
        S = b; // b là số chính phương gần EXP1 nhất
    }
            ...
            if (E2 % 2 == 1)
            {
                ...
                for (int i = 0; i < 500; i++)
                {
                    ...
                    else if (hp1 >= 200)
                    {
                        ...
                        {
                            // Thì Sherlock sẽ thực hiện nốt hành động này rồi thoát ra khỏi vòng lặp
                            hp1 = limitHP(roundUp(hp1 * 1.1)); // hp1 tăng lên 10%
                            break;
                        }
                        m1Used += 70;                      // Tiền sử dụng tăng 70
                        hp1 = limitHP(roundUp(hp1 * 1.1)); // hp1 tăng lên 10%
                        // nếu số tiền đã dùng lớn hơn 50% tiền ban đầu thì Sherlock sẽ không tiêu tiền trong đoạn đường 02 còn lại
                        if (m1Used > 0.5 * m1Initial)
                        {
                    ...
                    // Giai đoạn 2:
                    // Nếu exp1 của Sherlock nhỏ hơn 400 thì Sherlock sẽ chọn đi xe taxi:
                    if (exp1 < 400)
                    {
                        m1 -= 200; // tiền thực tế giảm 200
                        // Nếu tiền thực tế sau câu lệnh trên không là số dương
                        if (m1 <= 0)
                        {
                            // Thì Sherlock sẽ thực hiện nốt hành động này rồi thoát ra khỏi vòng lặp
                            exp1 = limitEXP(roundUp(exp1 * 1.13)); // exp1 tăng 13%
                            break;
                        }
                        m1Used += 200;                         // tiền sử dụng tăng 200
                        exp1 = limitEXP(roundUp(exp1 * 1.13)); // exp1 tăng 13%
                        // nếu số tiền đã dùng lớn hơn 50% tiền ban đầu thì Sherlock sẽ không tiêu tiền trong đoạn đường 02 còn lại
                        if (m1Used > 0.5 * m1Initial)
                        {
                    ...
                    // Nếu không thì exp1 không nhỏ hơn 400 thì Sherlock sẽ chọn đi xe ngựa:
                    else if (exp1 >= 400)
                    {
                        m1 -= 120; // tiền thực tế giảm 120
                        // Nếu tiền thực tế sau câu lệnh trên không là số dương
                        if (m1 <= 0)
                        {
                            // Thì Sherlock sẽ thực hiện nốt hành động này rồi thoát ra khỏi vòng lặp
                        ...
                        m1Used += 120;                         // tiền đã dùng tăng 120
                        exp1 = limitEXP(roundUp(exp1 * 1.13)); // exp1 tăng 13%
                        // nếu số tiền đã dùng lớn hơn 50% tiền ban đầu thì Sherlock sẽ không tiêu tiền trong đoạn đường 02 còn lại
                        if (m1Used > 0.5 * m1Initial)
                        {
                    ...
                    // Giai đoạn 3:
                    // Nếu exp1 của Sherlock nhỏ hơn 300 thì Sherlock sẽ nghe người dẫn đường hướng dẫn:
                    if (exp1 < 300)
                    {
                        m1 -= 100; // tiền thực tế giảm 100
                        // Nếu tiền thực tế sau câu lệnh trên không là số dương
                        if (m1 <= 0)
                        {
                            // Thì Sherlock sẽ thực hiện nốt hành động này rồi thoát ra khỏi vòng lặp
                            exp1 = limitEXP(roundUp(exp1 * 0.9)); // exp1 giảm 10%
                            break;
                        }
                        m1Used += 100;                        // tiền đã dùng tăng 100
                        exp1 = limitEXP(roundUp(exp1 * 0.9)); // exp1 giảm 10%
                        // nếu số tiền đã dùng lớn hơn 50% tiền ban đầu thì Sherlock sẽ không tiêu tiền trong đoạn đường 02 còn lại
                        if (m1Used > 0.5 * m1Initial)
                        {
                    ...
                    // Nếu không thì exp1 không nhỏ hơn 300 thì Sherlock sẽ nhờ người dẫn đường đi :
                    else if (exp1 >= 300)
                    {
                        m1 -= 120; // tiền thực tế giảm 120
                        // Nếu tiền thực tế sau câu lệnh trên không là số dương
...
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3)
{
    ...
    else
    {
        ...
        // Tương tự lặp 10 lần để đảm bảo xét hết các trường hợp
        for (int m = 0; m < 10; ++m)
        {
            // xét các ô phía dưới bên trái thì số hàng tăng 1, số cột tăng 1
            I += 1;
            J += 1;
            // Nếu số hàng lớn hơn 9 thì sẽ xảy ra lỗi, do đó thoát vòng lặp
            if (I > 9)
            {
                break;
            }
            // Nếu số cột lớn hơn 9 thì sẽ xảy ra lỗi, do đó thoát vòng lặp
            if (J > 9)
            {
                break;
            }
            // Kết quả sau cùng của B là giá trị lớn nhất của ô nằm trên nửa đường chéo trái dưới
        ...
        I = i;
        J = j;
        // Tiếp theo xét nửa dưới đường chéo phải:
        int C = matrix[I][J]; // C là giá trị lớn nhất của ô nằm trên nửa dưới đường chéo phải
        ...
        for (int m = 0; m < 10; ++m)
        {
            ...
            // Kết quả sau cùng của C là giá trị lớn nhất của ô nằm trên nửa dưới đường chéo phải
            C = my_max(C, matrix[I][J]);
        }

        // Reset giá trị của I và J để tiếp tục xét phần cuối cùng
        I = i;
        J = j;
