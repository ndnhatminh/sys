...
double probability(int a) {
    ...
    int Sus1 = (S-1)*(S-1);
    int Sus2 = S*S;
    if ((a - Sus1) < (Sus2 - a)) P = 1;
    else P = ( (a*1.0/Sus2) + 80)/123.0;
    return P;
}

void sumConverter2 ( int & x){ //Ham tinh tong 2 chu so
    x = (x/10) + (x%10);
}
...
int firstMeet(int & exp1, int & exp2, int e1) {
    ...
    {
        if (e1 <= 19) exp2 += roundUp(e1/4.0 + 19);
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    {
        int initialMoney = M1;
        if (reviewMoney(M1)) goto endoftheRoad1;
        ...
        if (M1 < (initialMoney * 0.5)) goto endoftheRoad1;
        ...
        if (M1 >= (initialMoney * 0.5)) goto EventLoop;
        else goto endoftheRoad1;
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            block[i][j] = ((E3*j) + (i*2))*(i-j);
            if (block[i][j] > E3*2) xOrder++;
            if (block[i][j] < -E3) yOrder++;
        }
    }
    while ( xOrder >= 10 )
    {
        sumConverter2(xOrder);
    }
    while ( yOrder >= 10 )
    {
        sumConverter2(yOrder);
    }

    int maxNumber = block[xOrder][yOrder];
    
    int a = xOrder - yOrder;
    int b = 0;
    if (a >= 0){
        while (a < 10){
            if (block[a][b] >= maxNumber) maxNumber = block[a][b];
            a++; 
            b++;
        }
    }
        ...
        while (-a < 10){
            if (block[b][-a] >= maxNumber) maxNumber = block[b][-a];
            a--; 
            b++;
        }
    }
        ...
        {
            while ( c >= 0){
                if (block[c][d] >= maxNumber) maxNumber = block[c][d];
                d++;
                c--;
            }
        }
        else
        {
            int f = 9;
            while (c - 9 <= 9){
                if( block[f][c-9] >= maxNumber ) maxNumber = block[9][c-9];
                c++;
                f--;
            }
        }
        if (absoluteValue(maxNumber) < absoluteValue(block[xOrder][yOrder])) {
