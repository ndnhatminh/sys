...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
    ...
    int tong = 0;
    int S;
    int can_bac_hai_duoi =(int)floor( sqrt(EXP1));
    ...
    if (E2 % 2 == 1)
    {
        ...
        EXP1+=(int)ceil((double)EXP1*17.0/100);     
        HP1-=(int)floor((double)HP1*17.0/100); 
        HP1 = (HP1 > 666) ? 666 : (HP1 < 0) ? 0 : HP1;
        EXP1 = (EXP1 > 600) ? 600 : (EXP1 < 0) ? 0 : EXP1;
    }
    else
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    int maxVal = a[i_meet ][j_meet ];
    int i_meet_start = i_meet;
    int j_meet_start = j_meet;
    while (i_meet_start  < 10 && j_meet_start  < 10)
    {
        maxVal = max(maxVal, a[i_meet_start ][j_meet_start ]);
        i_meet_start++;
        j_meet_start++;
    }
    i_meet_start = i_meet;
    ...
    while (i_meet_start  < 10 && j_meet_start  >= 0)
    {
        maxVal = max(maxVal, a[i_meet_start ][j_meet_start ]);
        i_meet_start++;
        j_meet_start--;
    ...
    i_meet_start = i_meet;
    j_meet_start = j_meet;
    while (i_meet_start >= 0 && j_meet_start  < 10)
    {
        maxVal = max(maxVal, a[i_meet_start ][j_meet_start ]);
        i_meet_start--;
        j_meet_start++;
    ...
    i_meet_start = i_meet;
    j_meet_start = j_meet;
    while (i_meet_start  >= 0 && j_meet_start  >= 0)
    {
        maxVal = max(maxVal, a[i_meet_start ][j_meet_start ]);
        i_meet_start--;
        j_meet_start--;
        ...
        EXP2 -= (int)floor((12.0/100 * (double)EXP2));
            ...
            EXP2 = (EXP2 > 600) ? 600 : (EXP2 < 0) ? 0 : EXP2;
            HP2 = (HP2 > 666) ? 666 : (HP2 < 0) ? 0 : HP2;
            kqcuoi= a[i_meet][j_meet];
        }
        else if (abs(a[i_meet][j_meet]) <= abs(maxVal))// neu duoi kip
        {
...
// Tìm độ dài của chuỗi s
    ...
    char se[101]; //  độ dài tối đa của phần trước '@' là 100
