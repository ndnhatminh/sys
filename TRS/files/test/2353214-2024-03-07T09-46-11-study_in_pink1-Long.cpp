...
int checkpass( string s, string email)  
{
    ...
    for (size_t i = 0; i < s.length(); ++i) {
        if (!std::isalnum(s[i]) && !Charactercheck(s[i])) {
            return i;
        }
    }
...
/*RUNNER TASK 5
const int MAX_PWDS = 30;
int password( const string  arr_pwds[],int num_pwds)
{
    int password_freq[MAX_PWDS] = {0};
    ...
    for (int i=0; i<num_pwds; ++i)
    {
        if(password_freq[i] > max_freq|| (password_freq[i]==max_freq && arr_pwds[i].length() > max_length)) {
    max_freq = password_freq[i];
    max_length = arr_pwds[i].length();
