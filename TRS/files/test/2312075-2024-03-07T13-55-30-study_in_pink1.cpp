...
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3) {
    ...
    for (int j = 0; j < size; j++) {
        for (int i = 0; i < size; i++) {
            ...
            // Lấy giá trị lớn nhất và lấy giá trị tuyệt đối nếu âm
    ...
    if (catch_up) {
        EXP1 = ceil((float)EXP1 + (float)EXP1*(12.0/100.0));
        VoidEXP(EXP1); 
        HP1 = ceil((float)HP1 + (float)HP1*(10.0/100.0));
        VoidHP(HP1);
        EXP2 = ceil((float)EXP2 + (float)EXP2*(12.0/100.0));
         VoidEXP(EXP2);
        HP2 = ceil((float) HP2 + (float)HP2*(10.0/100.0));
        VoidHP(HP2);
        return sw[i][j];
    } else {
        EXP1 = ceil((float)EXP1 - (float)EXP1*(12.0/100.0));
...
int checkPassword(const char *s, const char *email) {
    ...
    // Kiểm tra mk có chứa se hay không
