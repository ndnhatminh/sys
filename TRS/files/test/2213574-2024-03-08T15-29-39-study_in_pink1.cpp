...
{
    // TODO: Complete this function
    string emailStri = email;
    string sStri = s;
    string se = emailStri.substr(0, emailStri.find('@'));
    size_t sei = sStri.find(se);
    char element;
    ...
    for (int i = 0; i < sStri.length() - 2; i++)
    {
        if (sStri[i] == sStri[i + 1] && sStri[i + 1] == sStri[i + 2])
        {
            return -(400 + i);
        }
    }
    ...
    for (int i = 0; i < sStri.length(); i++)
    {
        if (!isalnum(sStri[i]) && !isSpecial(sStri[i]))
            return -5;
    }
