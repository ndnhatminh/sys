...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    {
        HP1 += ceil(double(HP1 * 0.1)); // cong them 10%
        m += 70;
        if (HP1 > 666) HP1 = 666;
    }
        }
...
bool chucai(char sh) {
    bool chuhoa = (('A' <= sh) && (sh <= 90));
    bool chuthuong = (('a' <= sh) && ('a' <= 122));
    if (chuhoa || chuthuong) return true;
    else return false;
}

bool chuso(char sh) {
    if ('0' <= sh & sh <= '9') return true;
    return false;
}
// có kí tự đặc biệt cho phép hay không
bool kytudacbiet(char sh) {
    if (sh == '!' || sh == '@' || sh == '#' || sh == '$' || sh == '%') return true;
    return false;
}
