...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
    ...
    //duong1
    if (HP1==0 && EXP1==0 && M1==0 && E2==0) return 0;
        if (EXP1 < 0) EXP1 = 0;
        if (EXP1 > 600) EXP1 = 600;
        if (HP1 > 666) HP1 = 666;
    q = round(sqrt(EXP1));
    if (EXP1 >= q * q) p1 = 100;
    else if ( (EXP1 < q*q) || (EXP1 == 0))p1 = round((float)((100 * EXP1) / (q * q) + 8000) / 123);
    // duong 2
    int MMM = M1;
    int m = 0;
    if (E2 % 2 == 0) {
    ...
    else if ( (EXP1 < q1*q1) || (EXP1 == 0))p2 = round((float)((100 * EXP1) / (q1 * q1) + 8000) / 123);
