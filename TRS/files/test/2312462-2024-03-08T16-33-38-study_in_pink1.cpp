...
int checkPassword(const char* s, const char* email) {
    ...
    // Yeu cau hop le cua mat khau
    const int minLength = 8;
    const int maxLength = 20;

    // Lay chuoi truoc ky tu '@' trong email
    ...
    // Kiem tra mat khau co chua chuoi se hay khong
    int sei = s1.find(se);
    if (sei != string::npos) return -(300 + sei);

    // Kiem tra mat khau co chua nhieu hon 2 ky tu lien tiep va giong nhau
    for (int i = 0; i < s1.length() - 2; i++) {
        if (s1[i] == s1[i + 1] && s1[i] == s1[i + 2]) return -(400 + i);
    }

    // Kiem tra mat khau co chua it nhat 1 ky tu đac biet
    bool check = false;
    ...
    //Kiem tra tung ki tu trong mat khau
    for (int i = 0; i < s1.length(); i++)
    {
        char c = s1[i];
