...
int firstMeet(int & exp1, int & exp2, int e1) {
    ...
    else exp1 = ceilf(exp1 - (float)D / 100);
    if (exp1>600){
        exp1=600;
    }
    if(exp1<0){
    ...
    if (80 <= e1 && e1 <= 99) {
        exp2 = ceilf(exp2 + (float)e1 / 4 + 19 + (float)e1 / 9 + 21);
        if (exp2 > 400) {
            exp2 = ceilf(exp2 + (float)e1 / 16 + 17);
            exp2 = ceilf(1.15 * (float)exp2);
        }
        exp1 = exp1 - e1;
    }
    if(exp1>600){
...
int checkPassword(const char * s, const char * email) {
   int lenS = strlen(s);
    ...
    int lense = strlen(se);
    char x[100]; 
        strncpy(x, email, se - email);
        x[se - email] = '\0'; 
        if (strstr(s, x) != NULL) {
        	const char* see = strstr(s, x);
        	int lensee = strlen(see);
           return -(300+(see-s)); 
        }      
