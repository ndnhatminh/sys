...
// Tinh toan va tra ve gia tri dua tren EXP1. Dau tien, tim so chinh phuong gan EXP1 nhat. Neu EXP1 lon hon hoac bang so chinh phuong do, tra ve 100. Ngoc lai, tra ve gia tri duoc tinh tu cong thuc cho truoc.
bool sqare(int n)
{
...
double traceLuggage_Street1(int EXP1)
{
    ...
    if (sqare(EXP1)) {
        ...
        if (sqare(s1)) {
            ...
            //double P1=(((double(EXP1)/s2)+80)/123);
            return (((double(EXP1)/s2)+80)/123)*100;
        }
    }
    while (1<0);
    return (((double(EXP1)/s2)+80)/123)*100;
...
{
    if (M1==0) return traceLuggage_Street1(EXP1); 
    else {
