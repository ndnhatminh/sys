...
void checkHP(float& HP1, float& HP2) {
...
void checkEXP(float& EXP1, float& EXP2) {
	// In case the index over limit
	if (EXP1 > 600) {
		EXP1 = 600;
	}
...
int traceLuggage(int& hp1, int& exp1, int& m1, int E2) {
	// TODO: Complete this function
	//Change the data type from intergers to float numbers
	float EXP1 = exp1, HP1 = hp1, M1 = m1;
	//Assign the the index which don't have in parameters
	float HP2 = 0, EXP2 = 0, M2 = 0;
	if (E2 > 99 || E2 < 0) {
		return -99;
	}
	checkHP(HP1, HP2);
...
outevent:
	...
	float arrP[20] = { 32,47,28,79,100,50,22,83,64,11 };
	int i = 0;
	float P3;
	if (E2 / 10 == 0) {
		i = E2;
