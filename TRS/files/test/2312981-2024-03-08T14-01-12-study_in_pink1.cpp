...
while (E2%2!=0){
 ...
 checkHP(HP1);
  ...
  //  std::cout << "matranTaxiPkt: " << std::endl;
    for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 10; ++j) {
            taxiPkt[i][j] = ((E3*j)+(i*2))*(i-j);
            //    std::cout   << taxiPkt[i][j] << "\t " ;
 ...
 // Khởi tạo và tính toán mảng maxTaxi
    long long maxTaxi[10][10] = {0};
       // std::cout << "matranmaxTaxi: " << std::endl;
...
int checkPassword(char const* s, char const* email) {
    ...
    if (password.length() < 8) {
        //std::cout << "do dai: " << password.length()<< std::endl;
    ...
    // Kiểm tra chưa chuỗi se 
    size_t sei = password.find(se);
    //std::string::pos trả về "no match" theo cplusplus.com => phủ định của no match là match :))
    ...
    for (char c : password) {//dùng lệnh for-each loop của C++11
        if (c == '@' || c == '#' || c == '%' || c == '$' || c == '!') {
            cokytudacbiet = true;
            break;
        }
...
int findCorrectPassword(const char* arr_pwds[], int num_pwds) {
    ...
    for (int i = 0; i < num_pwds; ++i) {
        ...
        for (int j = 0; j < num_pwds; ++j) {
            if (strcmp(arr_pwds[i], arr_pwds[j]) == 0) {
                info[i].count++;
               // std::cout << "căn bản là số lần của từng cái: "<<info[i].count << " " <<std::endl;
            }
        }
    }
    ...
    for (int i = 0; i < num_pwds; ++i) {
        if (info[i].count > max_count || (info[i].count == max_count && info[i].length > max_length)) {
            ...
            //std::cout <<"max so lan: "<< max_count<<std::endl;
            max_length = info[i].length;
            //std::cout <<"max dai: "<< max_length<<std::endl;
