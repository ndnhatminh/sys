...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    // Kiểm tra nếu tất cả các con đường đều có xác suất là 100%
    bool all100 = true;
    for (int i = 0; i < 3; ++i) {
        if (P[i] != 1.0) {
            all100 = false;
...
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3) {
    ...
    while (nPo >= 10)
    {
        meet_i = (nPo/10) + (nPo%10);
        return meet_i;
    }
    while (nNe >= 10)
    {
        meet_j = (nNe/10) + (nNe%10);
        return meet_j;
    }

    int meetScore = 0;
    int max_left_d = taxiMap[meet_i][meet_j];
    for (int i = 0; i < 10; i++) {
    if (taxiMap[i][i] > max_left_d) {
        max_left_d = taxiMap[i][i];
    }
    }
    int max_right_d = taxiMap[meet_i][meet_j];
    for (int i = 0; i < 10; i++) {
    if (taxiMap[i][9 - i] > max_right_d) {
        max_right_d = taxiMap[i][9 - i];
    }
    }
    meetScore = abs(max(max_left_d, max_right_d)); 

    // Cập nhật EXP và HP tương ứng
    if (abs(taxiMap[meet_i][meet_j]) < meetScore) {
        EXP1 = ceil(EXP1 * 1.12);
        HP1 = ceil(HP1 * 1.10);
        EXP2 = ceil(EXP2 * 1.12);
        HP2 = ceil(HP2 * 1.10);
        result = taxiMap[meet_i][meet_j];
    } else {
        EXP1 = ceil(EXP1 * 0.88);
        HP1 = ceil(HP1 * 0.9);
        EXP2 = ceil(EXP2 * 0.88);
...
int checkPassword(const char *s, const char *email) {
    ...
    // Kiểm tra mật khẩu có chứa ít nhất 1 ký tự đặc biệt không
    ...
    for (int i = 0; i < len; ++i) {
        ...
        // Kiểm tra mỗi ký tự có thuộc các loại cho phép không
...
int findCorrectPassword(const char *arr_pwds[], int num_pwds) {
    ...
    // Mảng đếm số lần xuất hiện của mỗi mật khẩu
    int count[30] = {0};

    // Duyệt qua mảng mật khẩu và đếm số lần xuất hiện của mỗi mật khẩu
