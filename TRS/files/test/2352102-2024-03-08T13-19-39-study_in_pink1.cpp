...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    int dem = 0;
    bool k[num_pwds]; // Xác định đã kiểm tra
    for (int i = 0; i < num_pwds; i++) {
        k[i] = false;
    }

    int b[num_pwds]; // Đếm số lần lặp lại
    for (int i = 0; i < num_pwds; i++) {
        b[i] = 0;
    }
    ...
    for (int i = 0; i < num_pwds - 1; i++) {
        for (int j = i + 1; j < num_pwds; j++) {
            if (strcmp(arr_pwds[i], arr_pwds[j]) == 0 && (k[i] != true || k[j] != true)) {
                b[i] += 1; // Đếm số lần lặp lại
                k[j] = true; // Trả về giá trị true để không phải kiểm tra lại
                k[i] = true;
            }
        }
    }
    ...
    // Nếu là 2 ký tự có cùng số lần lặp lại thì so sánh độ dài
    if (vtr2 != -1) {
    ...
    // Trường hợp tất cả các giá trị có cùng số lần lặp lại và độ dài của nó = 1
    if (poi == num_pwds - 1 && k[poi] != true) {
        poi = -1;
    }
