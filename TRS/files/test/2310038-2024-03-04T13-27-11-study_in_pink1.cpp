...
int firstMeet(int & EXP1, int & EXP2, int E1) {
    ...
    int D=e1*3+exp1*7;
    if (0<=e1&&e1<4&&D%2==0) exp1=ceil(exp1+(e1*3+exp1*7)/200-0.001);
    if (0<=e1&&e1<4&&D%2==1) exp1=ceil(exp1-(e1*3+exp1*7)/100-0.001);
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
 float S,m=M1,p,exp1=EXP1,hp1=HP1,m1=M1;
    ...
    p=(P1+P2+P3)/3;
    if (p==100) exp1=ceil(exp1*0.75-0.001); else
    if (p<50) {exp1=ceil(exp1*1.15-0.001); hp1=ceil(hp1*0.85-0.001);} else
