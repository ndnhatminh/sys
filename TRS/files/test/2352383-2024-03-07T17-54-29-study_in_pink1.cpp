...
int checkPassword(const char *s, const char *email) {
  // Extract se string from email
  const char *se = strchr(email, '@');
  if (!se) {
    return -7; // Invalid email format (no '@')
  }
  se++; // Skip the '@'
  ...
  // Check for se substring
  if (strstr(s, se) != nullptr) return -(300 + (se - email));
  ...
  for (int i = 0; i < len; i++) {
    if (!isalnum(s[i]) && s[i] != '@' && s[i] != '#' && s[i] != '%' && s[i] != '$' && s
        [i] != '!') {
      return -6;
    }
  }
