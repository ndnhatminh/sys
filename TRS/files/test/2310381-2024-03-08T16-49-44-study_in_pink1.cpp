...
int firstMeet(int & EXP1, int & EXP2, int E1)
{
    ...
    else if (E1<=3)
    {
        switch(E1)
        {
            case 0: 
            {
                EXP2+=29; checkExp(EXP2);
                break;
            }
            case 1: 
            {
                EXP2+=45; checkExp(EXP2);
                break;
            }
            case 2: 
            {
                EXP2+=75; checkExp(EXP2);
                break;
            }
            case 3:
            {
                EXP2+=29+45+75; checkExp(EXP2);
                break;
            }
        }
        int d=E1*3+EXP1*7;
        if (d%2==0)
        {
            EXP1=ceil((double) EXP1+1.0*d/200); checkExp(EXP1);
        }
        else
        {
            EXP1=ceil((double) EXP1-1.0*d/100); checkExp(EXP1);
        }
    }
    else
    {
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2)
{
    ...
    if (E2%2==1)
    {
        ...
        while (true)
        {
            if (HP1<200)
            {
                HP1=ceil(HP1*1.3-0.001); checkHp(HP1);
                totalM+=150; 
                if (totalM>lim)
                {
                    HP1=ceil(HP1*0.83-0.001); checkHp(HP1);
                    EXP1=ceil(EXP1*1.17-0.001); checkExp(EXP1);
                    break;
                }
            }
            else
            {
                HP1=ceil(HP1*1.1-0.001); checkHp(HP1);
                totalM+=70;
                if (totalM>lim)
                {
                    HP1=ceil(HP1*0.83-0.001); checkHp(HP1);
            ...
            {
                totalM+=200;
                EXP1=ceil(EXP1*1.13-0.001); checkExp(EXP1);
                if (totalM>lim)
                {
                    HP1=ceil(HP1*0.83-0.001); checkHp(HP1);
                    EXP1=ceil(EXP1*1.17-0.001); checkExp(EXP1);
            ...
            else
            {
                ...
                if (totalM>lim)
                {
                    HP1=ceil(HP1*0.8-0.001); checkHp(HP1);
                    EXP1=ceil(EXP1*1.17-0.001); checkExp(EXP1);
                    break;
                }
            }
            ...
            EXP1=ceil(EXP1*0.9-0.001); checkExp(EXP1);
        }
        M1-=totalM;
    }
    else
    ...
    {
        HP1=ceil(HP1*0.85-0.001); checkHp(HP1);
        EXP1=ceil(EXP1*1.15-0.001); checkExp(EXP1);
    }
    else
    {
        HP1=ceil(HP1*0.9-0.001); checkHp(HP1);
        EXP1=ceil(EXP1*1.2-0.001); checkExp(EXP1);
    }
    return HP1+EXP1+M1;
}
