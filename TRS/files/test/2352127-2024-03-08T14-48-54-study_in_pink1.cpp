...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    // TODO: Complete this function
     if (E3 >= 0 && E3 <= 99) {
        ...
        //Xet duong cheo
        for (i = 0; i < (10 - dem_i) && i < (10 - dem_j);i++) {
            if (taxi[dem_i + i][dem_j + i] > max) {max = taxi[dem_i + i][dem_j + i];}
        }
        for (i = 0; i < (10 - dem_i) && i <= dem_j;i++) {
            if (taxi[dem_i + i][dem_j - i] > max) {max = taxi[dem_i + i][dem_j - i];}
        }
        for (i = 0; i <= dem_i && i <= dem_j;i++) {
            if (taxi[dem_i - i][dem_j - i] > max) {max = taxi[dem_i - i][dem_j - i];}
        }
        for (i = 0; i <= dem_i && i < (10 - dem_j);i++) {
            if (taxi[dem_i - i][dem_j + i] > max) {max = taxi[dem_i - i][dem_j + i];}
        }
        int kq;
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    ...
    for (int i = 0; i < num_pwds; ++i) {
        ...
        if (dem > max_dem || (dem == max_dem && strlen(arr_pwds[i]) > max_do_dai)) {
            max_dem = dem;
            max_do_dai = strlen(arr_pwds[i]);
            max_dem_pwd = arr_pwds[i];
        }
    }
    for (int i = 0; i < num_pwds; ++i) {
        if (strcmp(arr_pwds[i], max_dem_pwd) == 0) {
            return i;
        }
    }
    return -1;
