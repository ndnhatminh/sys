...
int firstMeet(int & exp1, int & exp2, int e1) {
  ...
  if ((e1 > 99) || (e1 < 0)) return -99;
    ...
    if (e1 < 4) {
		...
		d = e1*3 + exp1*7;
		if (d % 2 == 0) { 
			exp1 = round((exp1 + double(d)/200) + 0.5);
		} 
		else { 
			exp1 = round(exp1 - double(d)/100 + 0.5);
		};
	}
	else {
		if (e1 < 20) {exp2 = round(exp2 + double(e1)/4 + 0.5) + 19;}
		else if (e1 < 50) {exp2 = round(exp2 + double(e1)/9 + 0.5) + 21;}
			else if (e1 < 66) {exp2 = round(exp2 + double(e1)/16 + 0.5) + 17;}
			 	else if (e1 < 80) {
				   		exp2 = round(exp2 + double(e1)/4 + 0.5) + 19;
				   		if (exp2 > 200) {exp2 = round(exp2 + double(e1)/9 + 0.5) + 21;};
				   	} else if (e1 < 100) {
				   			exp2 = round(exp2 + double(e1)/4 + 0.5) + 19;
				   			exp2 = round(exp2 + double(e1)/9 + 0.5) + 21;
				   			if (exp2 > 400) {exp2 = round(exp2 + double(e1)/16) + 17;};
				   			exp2 = round((double(exp2)*1.15) + 0.5);
					    }
		exp1 -= e1;
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    //conclution
    float p;
    if ((p1==100) && (p2==100) && (p3==100)) EXP1 = ceil_s(double(EXP1) * 0.75 + 0);
    else {
    	p = (p1 + p2 + p3) /  3;
    	if (p < 50) {
