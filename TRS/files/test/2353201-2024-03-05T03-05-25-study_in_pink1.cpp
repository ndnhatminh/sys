...
int checkPassword(const char* s, const char* email) {
    ...
    int sizeEmail, sizeS; int sizeSe = 0;
    ...
    for (int i = 0; i < sizeS; i++) {
        if (s[i] == 33 || (s[i] >= 35 && s[i] <= 37) || s[i] == 64) count++;
    }
    if (count == 0) return -5;

    //  Remaining cases
    for (int i = 0; i < sizeS; i++) {
        if (!(s[i] == 33 || (s[i] >= 35 && s[i] <= 37) || s[i] == 64 || (s[i] >= 48 && s[i] <= 57) || (s[i] >= 65 && s[i] <= 90) || (s[i] >= 97 && s[i] <= 122))) return i;
    }
