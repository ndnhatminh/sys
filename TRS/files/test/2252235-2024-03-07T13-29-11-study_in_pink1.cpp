...
int firstMeet(int & exp1, int & exp2, int e1) {
    ...
    if (e1 < 4){
        ...
        int d = e1*3 + exp1*7;
        if (d%2) exp1 -= d/100;
        else exp1 += (d%200 == 0? d/200: d/200 + 1);
    }
    else {
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    // conclusion
    if (P1 == 1 && P2 == 1 && P3 == 1) {EXP1 = (EXP1%4 == 0? EXP1*3/4: EXP1*3/4 + 1); return HP1 + EXP1 + M1;}
    if ((P1+P2+P3)*2/3 < 1) {
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    int j = 8;
    int taxis = (E3*j + 2*i)*(i - j);

    // sherlock
    int sherlock = 0; if (i == 9) goto caught;
    if (E3 > 2){
...
caught:
    ...
    EXP1 += (EXP1%25 == 0? EXP1*3/25: EXP1*3/25 + 1); if (EXP1 > 600) EXP1 = 600;
    HP2 += (HP2%10 == 0? HP2/10: HP2/10 + 1); if (HP2 > 666) HP2 = 666;
    EXP2 += (EXP2%25 == 0? EXP2*3/25: EXP2*3/25 + 1); if (EXP2 > 600) EXP2 = 600;
if (i == 9) return taxis;
    return sherlock;
}
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds){
    ...
    for (int i = 0; i + max_appear <= num_pwds; i++){
