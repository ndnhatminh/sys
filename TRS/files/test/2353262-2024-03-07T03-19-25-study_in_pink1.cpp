...
void condition(int & in, int con) // condition for task 1
{
    if ( in < 0)
    in = 0;
    else {
...
// Task 2
float road1P(int &EXP1 )  // hàm tìm số chính phương và tính P1
{   int a; 
    int i=0; 
    float P1;
    while (i*i<EXP1)
...
int index(int &E2) // tìm giá trị index ( nếu số 2 chữ số, tính tổng rồi lấy hàng đơn vị = i)
{
    int i = 0;
    if (E2 < 10)
    {
...
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3)
{
    ...
    for (int i = 0; i < 10; i++) // tạo mảng cho đường đi taxi
    {
    for (int j = 0; j < 10; j++)
     {
    ...
    int pointDet = INT_MIN;  // giá trị nhỏ nhất trong pointDet
...
// Task 4
int countC(const char *string)     // đếm bao nhiêu chuỗi
{
    int charN = 0;
    while (*string != '\0')
    {
...
int checkC(const char *t) {         // điều kiện kiểm tra các ký tự thỏa
    for (int i = 0; t[i] != '\0'; i++) {
...
int checkPassword(const char * s, const char * email) {
    ...
    const char *d = strchr(email, '@'); // sự xuất hiện đầu tiên của @ trong email
    int t = d - email;
    char *se = new char[t + 1]; // Tạo mảng cho chuỗi se
    strncpy(se, email, t); 
    se[t] = '\0';
    ...
    if (pp_se_pass == NULL) // null con trỏ chưa chỉ đến giá trị nào
        p_se_pass = -99;
    else
...
// Task 5
int cCharacter(const char *c) //đếm số chuỗi trong cEmail
{
    int cEmail = 0;
    while (*c != '\0')
    {
    ...
    const char *password = arr_pwds[i];
        ...
        { 
            if (i != j) {                                  
                ...
                if (strcmp(another, password) == 0)          // 2 vị trí liền kề không giống nhau
                {      
                    count++;                 
                }
            }
