...
int Left_diagonal(int coor_x, int coor_y, int mat[10][10]){
    ...
    if (coor_x > coor_y)
        for (int x = coor_x - coor_y, y = 0; x < 10; ++x, ++y)
            max_left = max(max_left, mat[x][y]);
    return max_left;
}
...
int Right_diagonal(int coor_x, int coor_y, int mat[10][10]){
    ...
    for (x = i, y = j; x > y; --x, ++y)
        max_right = max(max_right, mat[x][y]);
    return max_right;
}
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds){
	...
	int i_temp,
        i, j, 
        min_occurrence = -1, max_cnt = 0, 
        cnt = 1, index = 0,
        frequency_sorted[num_pwds] = {0};
    ...
    if (num_pwds == 1) return 0;
        ...
        for (i = 0; i + 1 < num_pwds; ++i){
			if (pass[i] != pass[i + 1] && i + 2 == num_pwds){
				if (length < pass[i + 1].length() && max_cnt == 1)
					length = pass[i + 1].length();
				sorted[index] = pass[i + 1];
				frequency_sorted[index] = 1;
				index++;
			}
			if (pass[i] != pass[i + 1]){
				if (max_cnt < cnt){
        ...
        for (i = 0; i + 1 < index; ++i)
            for (j = i + 1; j < index; ++j)
                if (frequency_sorted[i] < frequency_sorted[j] || sorted[i].length() < sorted[j].length() && frequency_sorted[i] == frequency_sorted[j]){
                    i_temp = frequency_sorted[i];
                    frequency_sorted[i] = frequency_sorted[j];
                    frequency_sorted[j] = i_temp;
                    s_temp = sorted[i];
                    sorted[i] = sorted[j];
                    sorted[j] = s_temp;
                }
        for (i = 0; i < num_pwds; ++i){
            if (strlen(arr_pwds[i]) == length)
			    for (j = 0; j < index, frequency_sorted[j] == max_cnt; ++j)
                    if (arr_pwds[i] == sorted[j]){
                        min_occurrence = i;
                        break;                        
                    }
            if (min_occurrence != -1)
