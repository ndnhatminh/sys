...
int checkPassword(const char * s, const char * email) {
 //Tìm ra chuỗi 'se'
    const char* backemail = strchr(email,'@');
 size_t length = backemail - email;
        char* se = new char[length + 1];
        strncpy(se, email, length);
        se[length] = '\0';
 //Kiểm tra chuỗi 's' có chứa chuỗi 'se' không ?
 const char* find = strstr(s, se);
 int sei = find - s;
 //Kiểm tra xem chuỗi 's' có >2 kí tự trùng nhau không
 int dai=strlen(s),h=0,sci;
 for (int i=0;i<=dai-3;i++)
 {if (s[i]==s[i+1]&&s[i+1]==s[i+2]) {h=h+1;sci=i;break;} }
 //Kiểm tra chuỗi 's' có kí tự đặc biệt không ?
 char arr[5]={'@', '#', '%','$', '!'};
 int x=0;
 ...
 //Cụ thể là chuỗi 's' có chứa kí tự khác số, chữ hoa, chữ thường, kí tự đặc biệt không ?
 int z=-1;
 for (int i=0;i<=dai-1;i++)
 {
 ...
 //Lần lượt kiểm tra từng yêu cầu và trả về giá trị
 //Hoàn thành task4
if (strlen(s)<8) return -1;
else if (strlen(s)>20) return -2;
else if (find != nullptr) return -(300+sei);
...
//Tạo 1 mảng chứa các mật khẩu riêng biệt     
     int k=0; 
 string arr_pwds2[30];
...
if (strcmp(arr_pwds[i],arr_pwds[j])!=0&&j==i-1) {arr_pwds2[k+1]=arr_pwds[i];k++;}
}
}
//Đếm xem có bao nhiêu mật khẩu riêng biệt
int count=0;
while(arr_pwds2[count]!="")
{count++;
}
//Tạo 1 mảng lưu số lần xuất hiện tương ứng mỗi phần tử
int arr_pwds3[count];
for (int i=0;i<count;i++) {arr_pwds3[i]=0;}
for(int i=0;i<count;i++)
{for (int j=0;j<num_pwds;j++)
    {if (arr_pwds2[i]==arr_pwds[j]) arr_pwds3[i]+=1;
    }
}
//Tìm mật khẩu xuất hiện nhiều nhất (nếu cùng tần số thì xét độ dài lớn hơn, nếu cùng cả 2 thì lấy vị trí đầu tiên)
//Hoàn thành task5
int max =  arr_pwds3[0];
int vitri=0;
    for (int i = 1; i < count; i++) {
        if (arr_pwds3[i] > max) {
            max = arr_pwds3[i];vitri=i;
        }
        else if (arr_pwds3[i] == max&&arr_pwds2[i].size()>arr_pwds2[vitri].size())
        {max=arr_pwds3[i];vitri=i; }
    }
for (int i=0;i<num_pwds;i++) {if (arr_pwds2[vitri]==arr_pwds[i]) {vitri=i;break;}}    
    return vitri;
    return -1;
}
