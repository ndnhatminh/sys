...
int firstMeet(int & exp1, int & exp2, int e1) {
 ...
 if (e1 < 0 || e1 > 99){return -99;}
    if (e1==0){addExp(exp2 , 29); exp1=decision(e1,exp1);} //truong hop 0
     ...
     else if(e1<=19 && e1 >=4){addExp(exp2 , e1/4.0 + 19.0);addExp(exp1 , -e1);} //truong hop 2.1
     else if(e1<=49 && e1 >=20){addExp(exp2 , e1/9.0 + 21.0);addExp(exp1 , -e1);} //truong hop 2.2
     else if(e1<=65 && e1 >=50){addExp(exp2 , e1/16.0 + 17.0);addExp(exp1 , -e1);} //truong hop 2.3
     else if(e1<=79 && e1 >=66){addExp(exp2 ,  19); addExp(exp2 , e1/4.0);
          if(exp2 > 200){addExp(exp2 , e1/9.0 + 21.0);} //check > 200
              addExp(exp1 , -e1);}  //truong hop 2.4
     else if(e1<=99 && e1 >=80){addExp(exp2 , e1/4.0 + 19.0);addExp(exp2 , e1/9.0 +21.0) ;
          if(exp2 > 400){addExp(exp2 , 17.0 + e1/16.0);addExp(exp2 , 15.0/100.0 * exp2);exp2=check(exp2,0,600);} //check > 400
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    int pay=M1;
     do{
         if(HP1<200){addHp( HP1, HP1*(30.0/100.0));addM(M1 , -150);pay-=150;if(pay<0) break;} 
            else {addHp( HP1, HP1*(10.0/100.0));addM(M1 , -70);pay-=70;if(pay<0) break;}; 
    //truong hop 1
        if(EXP1<400){addM(M1 , -200);addExp(EXP1,13.0/100.0 * EXP1);pay-=200;if(pay<0) break;}
            else {addM(M1 , -120) ; addExp(EXP1,13.0/100.0 * EXP1);pay-=120;if(pay<0) break;};
    //truong hop 2
        if(EXP1<300){addM(M1 , -100) ; addExp(EXP1, -10.0/100.0 * EXP1);pay-=100;if(pay<0) break;}
            else{addM(M1 , -120) ; addExp(EXP1 , -10.0/100.0 * EXP1 );pay-=120;if(pay<0) break;}
    //truong hop 3
    break;
    }while(payM>0);
    //E2 la so chan
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
   ...
   for(int i=0;i<10;i++){
    ...
    if(duong>10){
         while(duong>=10){duong=duong/10 + duong%10;}
   }
   
    if(am>10){
         while(am>=10){am=am/10 + am%10;}
   }
...
int checkPassword(const char * s, const char * email) {
    ...
    string mk(s); //chuyen const char thanh string
    
    
    int counte = 0;
    while (email[counte] != '\0') {
        counte++;
    }
    string e(email); //chuyen const char thanh string
    

    //Finding se
    int position = e.find('@');
    string se = e.substr(0, position); // xac dinh phan truoc @
    //check < 8
    if (counts < 8) {return -1;}
    //check >20
    if (counts > 20) {return -2;} 

    //check co chua string e hay ko
    if (mk.find(se) != string::npos) {return -(300 + mk.find(se));}

    //having over-double symbol?
    for (int i = 0; i < counts - 3; i++) {
        if (mk[i] == mk[i+1] && mk[i+1] == mk[i+2]) {return -(400 + i);}
    }
    ...
    for (int i = 0; i < counts; i++) {  
        if (!((mk[i] >= '0' && mk[i] <= '9') || (mk[i] >= 'a' && mk[i] <= 'z')  || (mk[i] >= 'A' && mk[i] <= 'Z')  || (mk[i] == '@' || mk[i] == '#'   || mk[i] == '%' || mk[i] == '$' || mk[i] == '!'))) {
            return i; 
        }
    } 
    return -10;  
