...
int firstMeet(int& exp1, int& exp2, int e1) {
	...
	if (e1 >= 0 && e1 <= 3 && e1 != -99) {
		...
		int d = e1 * 3 + exp1 * 7;
		if (d % 2 == 0) {
			//exp1 = ceil(1.0 * exp1 + (1.0 * d / 200));
			exp1 += ceil((round(10000.0 * d)/10000) / 200);
			//cout <<"a: "<< exp1<<endl;
			check(exp1, 0, 600);
		}
		else {
			//exp1 = ceil(1.0 * exp1 - (1.0 * d / 100));
		...
		if (4 <= e1 && e1 <= 19) {
			...
			check(exp2, 0, 600);
			if (exp2 > 400) {
				//exp2 += ceil(1.0 * e1 / 16 + 17);   //chu y hay bi sai o day khi lam tron				
				exp2 += ceil((round(10000.0 * e1) / 10000) / 16 + 17);
				check(exp2, 0, 600);
				//exp2 = ceil(1.15 * exp2);
...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
	...
	check(EXP1, 0, 600);
	// cout<<"exp1e: "<<EXP1<<endl;
	//HP1 = ceil(HP1 - int_float(HP1) * 0.17);
...
int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3) {
	...
	if (abs(taxi[n][m]) > sw[n][m]) {
		...
		//EXP1 = ceil(1.12 * EXP1);
		//EXP1 = ceil((round(10000.0 * EXP1) / 10000) * 1.12);
		EXP1 = ceil(round(1.12 * EXP1 * 10000) / 10000);
		check(EXP1, 0, 600);
		//cout << "exp1b: " << EXP1 << endl;
		//EXP2 = ceil(1.12 * EXP2);
		//EXP2 = ceil((round(10000.0 * EXP2) / 10000) * 1.12);
		EXP2 = ceil(round(1.12 * EXP2 * 10000) / 10000);
		check(EXP2, 0, 600);
		//cout << "exp2b: " << EXP2 << endl;
		//HP1 = ceil(1.1 * HP1);
		//HP1 = ceil((round(10000.0 * HP1) / 10000) * 1.1);
		HP1 = ceil(round(1.1 * HP1 * 10000) / 10000);
		check(HP1, 0, 666);
		//cout << "hp1b: " << HP1 << endl;
		//HP2 = ceil(1.1 * HP2);
		//HP2 = ceil((round(10000.0 * HP2) / 10000) * 1.1);
