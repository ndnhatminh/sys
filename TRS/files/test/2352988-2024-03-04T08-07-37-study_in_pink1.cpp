...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    if (b + 1 <= ROW - 1 && c + 1 <= COL - 1) max1 = a[b + 1][c + 1];
    if (b - 1 > 0 && c + 1 <= COL - 1) max2 = a[b - 1][c + 1];
    if (b + 1 <= ROW - 1 && c - 1 > 0) max3 = a[b + 1][c - 1];
    if (b - 1 > 0 && c - 1 > 0) max4 = a[b - 1][c - 1];
    while (b < ROW && c < COL){
	    b++;
	    c++;
	    if (b >= ROW - 1 || c >= COL - 1){
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    ...
    int n=0;
    for(n;n<num_pwds;n++){
        if(correct.compare(arr_pwds[n])==0){
            break;
        }
    }
    return n;
