...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
	...
	if (E2 % 2 != 0)
	{
		while (t >= 0)
		{
			...
			M1 = ceil(M1);
			if (t > n) break;
			if (EXP1 < 400) { M1 = M1 - 200; t = t + 200; u = u * 1.13; }
			else { M1 = M1 - 120; t = t + 120; u = u * 1.13; }
			EXP1 = ceil(u);
			u = EXP1;
			M1 = ceil(M1);
			if (t > n) break;
			if (EXP1 < 300) { M1 = M1 - 100; t = t + 100; u = u * 0.9; }
			else { M1 = M1 - 120; t = t + 120; u = u * 0.9; }
			EXP1 = ceil(u);
			u = EXP1;
			M1 = ceil(M1);
			if (t > n) break;
	...
	if (E2 % 2 == 0)
	{
		...
		y = HP1;
		M1 = ceil(M1);
		if (EXP1 < 400 && M1>0) { M1 = M1 - 200; t = t + 200; u = u * 1.13; }
		else if (EXP1 >= 400 && M1 > 0) { M1 = M1 - 120; t = t + 120; u = u * 1.13; }
		EXP1 = ceil(u);
		u = EXP1;
		M1 = ceil(M1);
		if (EXP1 < 300 && M1 > 0) { M1 = M1 - 100; t = t + 100; u = u * 0.9; }
		else if (EXP1 >= 300 && M1 > 0) { M1 = M1 - 120; t = t + 120; u = u * 0.9; }
		EXP1 = ceil(u);
		u = EXP1;
		M1 = ceil(M1);
		if (M1 < 0) M1 = 0;
	...
	if (p1 == 1 && p2 == 1 && p3 == 1) { u = u * 0.75; EXP1 = ceil(u); }
	else
	{
		double p = (p1 + p2 + p3) / 3;
		if (p < 0.5) { y = y * 0.85; u = u * 1.15; }
		else { y = y * 0.9; u = u * 1.20; }
		HP1 = ceil(y);
		EXP1 = ceil(u);
	}
