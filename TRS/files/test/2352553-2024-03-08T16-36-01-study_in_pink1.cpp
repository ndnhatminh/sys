...
void r1(int & EXP1x, double & P1x)  //hàm tính p1 hoặc p2
{
    ...
    if ( (EXP1x - l1*l1) <= (l2*l2-EXP1x))
        P1x = 1.0000000;
    else {P1x = (EXP1x / (l2*l2*1.00000) + 80) /123.000;  }
        return; 
}
//
...
{ 
 ...
 if(x1<0)x1=0;
        if(EXP1<400&&x1>=M1/2)
            {x1+=-200;EXP1=ceil(EXP1*1.12999);}
       else  if(EXP1>=400&&x1>=M1/2)
            {x1+=-120; EXP1=ceil(EXP1*1.12999);}
 if(EXP1>600)EXP1=600;
if(x1<0)x1=0;
        if(EXP1<300&&x1>=M1/2)
             {x1+=-100;EXP1=ceil(EXP1*0.8999);}
        else if (EXP1>=300&&x1>=M1/2)
           {x1+=-120;
            EXP1=ceil(EXP1*0.8999);}
...
if(HP1>666)HP1=666;
 ...
 if(EXP1<300&&x1>0)
           {x1+=-100; EXP1=ceil(EXP1*0.8999);}
 else if (EXP1>=300&&x1>0)
          {x1+=-120;  EXP1=ceil(EXP1*0.8999);}
 if (x1<=0) x1=0;
 if(EXP1>600)EXP1=600;
 }
 //chạy bộ
     HP1=ceil(HP1*0.82999);
     EXP1=ceil(EXP1*1.16999);
     if(x1 < 0) x1 = 0;
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
   ...
   if (E2<0||E2>99)return -99;
    ...
    if (P1 == 1.00 && P2 == 1.00 && P3 == 1.00){
        ...
        Pa = (P1 + P2 + P3) / 3.000000;
        if(EXP1>600)EXP1=600;
    if (Pa < 0.50)
    {
        HP1 =ceil(HP1*0.84999);
        EXP1 = ceil(EXP1*1.14999);
    }
    else
    {
        HP1 =ceil(HP1*0.8999);
        EXP1 = ceil(EXP1*1.1999);
    } 
    }
...
y=(cn/10+cn%10)/10+(cn/10+cn%10)%10;
 ...
 taxi=((E3 * y) + (x * 2)) * (x - y);
 max=0; // tìm max
for((i=x,j=y);i>0 && j < 10;(i-=1,j+=1))
{
    if(X[i][j]>max)max=X[i][j];

}
for((i=x,j=y);j>0 && i < 10;(i+=1,j-=1))
{
    if(X[i][j]>max)max=X[i][j];
...
for((i=x,j=y);i<=9&&j<=9;(i+=1,j+=1))
{
    ...
    if (abs(taxi)>max)
    {
        EXP1=ceil(EXP1*0.87999);
        EXP2=ceil(EXP2*0.87999);
        HP1=ceil(HP1*0.8999);
        HP2=ceil(HP2*0.8999);
        hpexp( EXP1, EXP2, HP1, HP2);
return taxi;
    }
    else
    {
         EXP1=ceil(EXP1*1.11999);
        EXP2=ceil(EXP2*1.11999);
        HP1=ceil(HP1*1.0999);
        HP2=ceil(HP2*1.0999);
        hpexp( EXP1, EXP2, HP1, HP2);
        return max;
...
if (strlen(s)<8) { return -1;} // so sánh đọ dài password  có lớn hơn 20 hay bé hơn 8 không?
 ...
 // kiểm tra se có trong s không và tìm tọa độ
  const char *pse = strstr(s, se); // khảo sát s tìm se
   ...
   bool flag = false;
   for(int i=0;i<strlen(s);i++) //kiểm tra kí tự đặc biệt
     {
...
//   for(int g=0;<strlen(s);g++) //kiểm tra kí tự đặc biệt
//      {
//           if (*s != '#' && *s != '%' && *s != '$' && *s != '!' && *s != '@')
//           return -5;
//      }
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    string mcp= arr_pwds[0];;
    int mc = 0;
    
    for (int i = 0; i < num_pwds; i++) {    
        int count = 1;
        for (int j = i + 1; j < num_pwds; j ++)  //tìm số lặp lớn nhất
            if (arr_pwds[i] == arr_pwds[j])
                count ++;
        if (count > mc) {  //tìm password dài nhất
            mc = count;
            mcp = arr_pwds[i]; // ghi lại kết quả cuối cùng
        }
        else if (count == mc){
            if (strlen(arr_pwds[i]) > mcp.length()){
                mc = count;
                mcp = arr_pwds[i];
            }
        }
