...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    count_j = 0;
    int i, j, a, b;
    int arr_taxi[10][10] = {}; //Tao mang cho taxi
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) {
            arr_taxi[i][j] = ((E3*j) + (i*2))*(i-j);
            if (arr_taxi[i][j] < (E3*2)) {count_i += 1;}
            if (arr_taxi[i][j] < ((-1)*E3)) {count_j += 1;}
        }
    }
    //Neu count_i hoac count_j co 2 chu so
    int E6;
    E6 = count_i;
    while (E6 >= 10) {
        E6 = ((count_i/10) + count_i%10)%10;
    }
    count_i = E6;
    E6 = count_j;
    while (E6 >= 10) {
        E6 = ((count_j/10) + count_j%10)%10;
    }
    count_j = E6;
    int arr_saw[10][10] = {}; //Tao mang cho sherlock va watson
    int max_value;
    max_value = arr_taxi[0][0];
    for (j = 0; j < 10; j++) {
        for (i = 0; i < 10; i++) {
            b = j;
            //Tim gia tri lon nhat tren duong cheo phai
            for ( a = i; a < 10; a++) {
                    if (max_value < arr_taxi[a][b]) {max_value = arr_taxi[a][b];}
                    b = b + 1;
            }
            b = j;
            for ( a = i; a = 0; a--) {
        ...
        arr_saw[i][j] = abs(max_value);
    //Tai vi tri gap nhau
    if (abs(arr_taxi[count_i][count_j]) > arr_saw[count_i][count_j]) {
    ...
    int tra_ve;
    if (abs(arr_taxi[count_i][count_j]) > arr_saw[count_i][count_j]) {
        tra_ve = arr_taxi[count_i][count_j];
    }
    else {
        tra_ve = arr_saw[count_i][count_j];
    }
    return tra_ve;
    }
    else {
