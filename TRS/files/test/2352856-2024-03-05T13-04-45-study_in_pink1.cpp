...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) 
{
    ...
    for (int a = 0; a < 10; a++)
        for (int b = 0; b < 10; b++) arrayTaxi[a][b] = 0;

    for (int a = 0; a < 10; a++)
        for (int b = 0; b < 10; b++)
        {
            arrayTaxi[a][b] = ((E3 * b) + (a * 2)) * (a - b);
            if (arrayTaxi[a][b] < (0-E3)) j_point++;
            if (arrayTaxi[a][b] > (E3 * 2)) i_point++;
            
        }

    while(i_point > 9) i_point = (i_point/10) + (i_point%10);
    while(j_point > 9) j_point = (j_point/10) + (j_point%10);

    int score = arrayTaxi[i_point][j_point];

    for (int a = 0; a < 10; a++)
        for (int b = 0; b < 10; b++) if ((a - b) == (i_point - j_point) || ((a + b) == (i_point + j_point)))            
            {
                if (arrayTaxi[a][b] > score) score = arrayTaxi[a][b];
            }

    if (abs(arrayTaxi[i_point][j_point]) > score)
    {
        ...
        EXP2 = checkexp(EXP2);
        HP1 = checkhp(HP1);
        return arrayTaxi[i_point][j_point];
    }
    else
    {
