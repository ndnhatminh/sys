    ...
    if(e1>=0&&e1<=3) {
        ...
        switch(e1) {
            ...
            case 3:
                ...
                break;}
        int D=3*e1+7*exp1;
        D%2==0?     exp1+=ceil((double)D/200) :   exp1-=floor((double)D/100);
        exp1=check1(exp1);
        return exp1 + exp2;}
    else if(e1>=4&&e1<=99) {
        if(e1<=19) {
            ...
            exp2+=ceil((double)e1/4)+19;
            exp2=check2(exp2);
            exp2>200?   exp2+=ceil((double)e1/9)+21 :   exp2=exp2;
            exp2=check2(exp2);}
        else if(e1<=99) {
            exp2+=ceil((double)e1/4)+19;
            exp2=check2(exp2);
            exp2+=ceil((double)e1/9)+21;
            exp2=check2(exp2);
            exp2>400?   exp2=ceil((exp2+ceil((double)e1/16)+17)*1.15):   exp2=exp2;
            exp2=check2(exp2);}
        exp1-=e1;
        exp1=check1(exp1);
        return exp1 + exp2;}
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if(E2<0||E2>99)    return -99;
        ...
        //Path 1
        int S1=findS(EXP1);
        EXP1>=S1?    P1=1    :   P1=(((double)EXP1/S1)+80)/123;
        //Path 2
        int m=floor((double)M1/2);
        int t=0;
        if(E2%2==0) {
            while(1) {
                if(M1==0)   break;
        ...
        EXP1=check1(EXP1);
        int S2=findS(EXP1);
        EXP1>=S2?    P2=1    :   P2=(((double)EXP1/S2)+80)/123;
        //Path 3
        int P[10]={32,47,28,79,100,50,22,83,64,11};
        int k=E2/10 + E2%10;
        P3=(double)P[k%10]*0.01;
        if(P1==1&&P2==1&&P3==1) {
