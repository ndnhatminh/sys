...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    HP1 = ceil(HP1*0.83f); //walk to end of road
    HP1 = checkHP(HP1);
    ...
    {
        int i = sumOfDigits(E2);
        P3 = float(P[i])/100;
    }
    if (P1 == 1 && P2 == 1 && P3 == 1) // mistake
    {
        EXP1 = ceil(0.75f*EXP1);
...
int checkPassword(const char * s, const char * email) {
    ...
    for (int i = 0; i < strlen(s); i++) 
    {
        ...
        if (!((s[strlen(s) - i - 1] >= 'a' && s[strlen(s) - i - 1] <= 'z') || (s[strlen(s) - i - 1] >= 'A' && s[strlen(s) - i - 1] <= 'Z') || (s[strlen(s) - i - 1] >= '0' && s[strlen(s) - i - 1] <= '9'))){
