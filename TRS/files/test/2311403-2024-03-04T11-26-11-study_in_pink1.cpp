...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if (EXP1 >= S) {
        ...
        P1 = (((float) EXP1 / S + 80.0) / 123.0) * 100; //65.84
    }
    // ROAD 2
    int paidMoney = 0;
    ...
    if (E2 % 2 != 0){
        ...
        while (M1 != 0){
            if (HP1 >= 200 ){
                ...
                HP1 = ceil((float) HP1 * 130 / 100); // 164
                modH(HP1);
                M1 -= 150;
                modM(M1);
                if (M1 == 0){
    ...
    // ADJUST HP AND EXP
    HP1 = ceil((float) HP1 * 83 / 100); // 137
    modH(HP1);
    EXP1 = ceil((float) EXP1 * 117 / 100); //168
    modE(EXP1);
    x = sqrt(EXP1);
    if (x * x == EXP1){
        S = EXP1;
    ...
    if (EXP1 >= S) {
        ...
        P2 = (((float) EXP1 / S + 80.0) / 123.0) * 100; // 65,84
    }
    // ROAD 3
    float P3;
    ...
    if (avg == 100){
        ...
        HP1 = ceil((float) HP1 * 85 / 100); //117
        modH(HP1);
        EXP1 = ceil((float) EXP1 * 115 / 100); //194
        modE(EXP1);
    }
    else {
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    ...
    for (int i = 0; i < num_pwds; i++){
        ...
        if (count == maxcount && strlen(arr_pwds[i]) > longestPassword.length()){
            longestPassword = arr_pwds[i];
            index = i;
        }
    }
    return index;
