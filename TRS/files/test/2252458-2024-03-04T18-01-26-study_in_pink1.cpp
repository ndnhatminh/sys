...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    M1 = check(M1, 0, 3000);
    S = EXP1;
    int P2 = getClosestPerfectSquare(S, EXP1);
    // road 3
...
// Task 3
 #include<vector>
 #include<algorithm>
vector<vector<int>> taxiMatrix(10, vector<int>(10, 0));
//vector<vector<int>> SherlockWatsonMatrix(10, vector<int>(10, 0));
...
int getScoreSherlockWatson(int i, int j) {
    ...
    int x = i, y = j;
    while (x >= 0 && y >= 0) {
        possiblePoints.push_back(taxiMatrix[x][y]);
        --x;
        --y;
    }
    x = i, y = j;
    ...
    return *max_element(possiblePoints.begin(), possiblePoints.end());
}
...
int checkPassword(const char * s, const char * email) {
    ...
    if (n < 8) return -1;
    if (n > 20) return -2;
    if (str.find(se) != string::npos) return -(300 + str.find(se));
    
    for (int i = 0; i < n - 2; ++i) {
        if (str[i] == str[i+1] && str[i+1] == str[i+2]) return -(400 + i);
    }
    
    if (str.find_first_of(special_chars) == string::npos) return -5;
    
    for (char c : str) {
        if (!isdigit(c) && !isalpha(c) && special_chars.find(c) == string::npos) {
            return str.find(c);
        }
    }
