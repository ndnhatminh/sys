...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) 
{
    if (E3 >= 0 && E3 <= 99)
    {
    // Khoi tao ma tran 10x10 voi vi tri va nap gia tri theo cong thuc ((E3 * j) + (i * 2)) * (i - j)
    ...
    // tong chu so cua 2 chu so cho den khi <9
    while (i >= 10) 
    {
        i = i / 10 + i % 10;
    }
...
int checkPassword(const char * s, const char * email) 
{
    ...
    // Khong tin thay se tra ve string::npos con neu tim thay thi bao loi
...
{
    // Khoi tao bien chi ve so lan xuat hien cua mat khau
    int max_soluong = 0;
    // Khoi tao bien chi ve chieu dai cua mat khau
    int max_chieudai = 0;
    // Khoi tao gia tri tra ve tuong ung voi mat khau so lan xuat hien nhieu nhat va dai nhat
    string mkdung;
