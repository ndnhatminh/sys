...
EXP1 = std::ceil(EXP1 - EXP1 * 0.12);
   ...
   // Task 4
    ...
    std::string kytudacbiet = "@#%$!";

    int dodai = password.length();
    if (dodai < 8)
        {
        return -1;
        }
    ...
    for (size_t sci = 0; sci < dodai - 2; sci++) {
    ...
    for (char s : password) {
    if (kytudacbiet.find(s) != std::string::npos) {
        checkktdb = true;
        break;
    }
}
    ...
    for (size_t i = 0; i < dodai; ++i) {
        char s = password[i];
        if (!isalnum(s) && kytudacbiet.find(s) == std::string::npos){
         return i;        
    }
    }
    return -10;
