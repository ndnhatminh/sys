  ...
  setthelimit(HP1, EXP1, M1);
    ...
    if (E2 % 2 != 0)
    {
        while (true)
        {
            if( HP1 < 200)
            {
                HP1 = ceil( (1.0*HP1) * (130.0/100) );
                M1 = M1 - 150;
               money += 150;
                setthelimit(HP1, EXP1, M1);
            } else 
            {
                HP1 = ceil( (HP1*1.0) * (110.0/100) );
                 M1 = M1 - 70;
                money += 70;
                setthelimit(HP1, EXP1, M1);
            } 
            
            if( money > half_of_money)
            {
                break;
            }
            ...
            {
            M1 = M1 - 100;
            EXP1 = ceil(( EXP1 * 1.0) - (10.0/100)*(EXP1*1.0));
            setthelimit(HP1, EXP1, M1);
            } else 
            {
            M1 = M1 - 120;
        ...
        HP1 = ceil((HP1 * 1.0) - (17.0/100)*(HP1 * 1.0));
        EXP1 = ceil(( EXP1 * 1.0) + (17.0/100)*(EXP1*1.0));
        setthelimit(HP1, EXP1, M1);
    }
    ...
    {
       P2 = ((EXP1*1.0/S2 + 80) / 123.0)*100.0;
    }
 
    double arr[10] = { 32.0, 47.0, 28.0, 79.0, 100.0, 50.0, 22.0, 83.0, 64.0, 11.0 };
    double P3, totalP;
        if (E2 >= 0 && E2 <= 9)
        {
           int i = E2;
    ...
    {
        EXP1 = ceil(( EXP1 * 1.0) - (25.0/100)*(EXP1*1.0));
        setthelimit(HP1, EXP1, M1);
    } else 
    {
        totalP = (P1 + P2 + P3) / 3;
    ...
    if ( totalP < 50.0s)
    {
         HP1 = ceil((HP1 * 1.0) - (15.0/100)*(HP1 * 1.0));
         EXP1 = ceil(( EXP1 * 1.0) + (15.0/100)*(EXP1*1.0));
         setthelimit(HP1, EXP1, M1);
    } else 
    {
        HP1 = ceil((HP1 * 1.0) - (10.0/100)*(HP1 * 1.0));
        EXP1 = ceil(( EXP1 * 1.0) + (20.0/100)*(EXP1*1.0));
        setthelimit(HP1, EXP1, M1);
    }
    return HP1 + EXP1 + M1;
}
...
int smaller = 0;
    ...
    {
        EXP1 = ceil((EXP1*1.0) + (12.0/100)*(EXP1*1.0));
         setthelimit(HP1, EXP1, M1);
        HP1  = ceil(HP1*1.0 + (10.0/100)*(HP1*1.0));
         setthelimit(HP1, EXP1, M1);
         EXP2 = ceil(EXP2*1.0 + (12.0/100)*(EXP2*1.0));
         setthelimit(HP2, EXP2, M1);
         HP2  = ceil(HP2*1.0 + (10.0/100)*(HP2*1.0));
          setthelimit(HP2, EXP2, M1);
