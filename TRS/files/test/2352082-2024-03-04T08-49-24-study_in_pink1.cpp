...
int coordinate(int x)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        {
            // Extract unit and tens digits of the number
            int unit = x % 10;
            int tenth = (x / 10) % 10;
            // Calculate the sum of unit and tens digits
            int sum = unit + tenth;
            // Return only the unit digit of the sum
            x = sum;
        }
    }
    return coordinate;
...
int firstMeet(int& exp1, int& exp2, int e1)
{
    ...
    else if (e1 >= 4 && e1 <= 99)
    {
        ...
        { // e1 in the range [80,99]
            // make sure exp2 round up correctly
            x = exp2 + (e1 / 4.0 + 19);
            exp2 = ceil(x);
            experience(exp2);
            x = exp2 + (e1 / 9.0 + 21);
...
int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            // tim toa do cua diem tren cung duong cheo trai va cheo phai tu diem xac dinh
            // cheo trai
            for (int z = 0; z < 10; z++)
            {
                if (row1 == 0 || col1 == 0)
            ...
            int largest_right = a[row2][col2];

            // find the largest
            //cheo trai
            for (int h1 = row1 + 1; h1 < 10; h1++)
            {
                for (int c1 = col1 + 1; c1 < 10; c1++)
                {
                    if (a[row1 + 1][col1 + 1] > largest_left)
                    {
                        largest_left = a[row1 + 1][col1 + 1];
                    }
                    else
                    {
                        row1++;
            ...
            //cheo phai
            for (int h2 = row2 + 1; h2 < 10; h2++)
            {
                for (int c2 = col2; c2 > 0; c2--)
                {
                    if (a[row2 + 1][col2 - 1] > largest_right)
                    {
                        largest_right = a[row2 + 1][col2 - 1];
                    }
                    else
                    {
                        row2++;
