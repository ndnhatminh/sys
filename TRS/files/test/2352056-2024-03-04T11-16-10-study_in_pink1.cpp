...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if (EXP1 >= S1) P1 = 1.0;
    else P1 = (static_cast<double>(EXP1) / static_cast<double>(S1) + 80.0) / 123.0;
    ...
    if (EXP1 >= S2) P2 = 1.0;
    else P2 = (static_cast<double>(EXP1) / static_cast<double>(S2) + 80.0) / 123.0;
...
int checkPassword(const char * s, const char * email) {
    ...
    if (Password.length() < 8) return -1;  // At least 8 characters
        ...
        for (int i = 0; i < Password.length(); i++) {  // At least 1 special character
            ...
            if (!(Char == '@' || Char == '#' || Char == '%' || Char == '$' || Char == '!' || (Char >= '0' && Char <= '9') || (Char >= 'a' && Char <= 'z') || (Char >= 'A' && Char <= 'Z'))) return i;  // No letters (lowercase and uppercase), numbers and given special characters
        }
