  ...
  int diag_max(int arr[10][10], int row, int col) {
        if (row < 0 || col < 0 || row >= 10 || col >= 10){
          return 0;
        }
        int dmax = arr[row][col];
        int row_increment = row;
        int col_increment = col;
        while (row_increment >= 0 && col_increment >= 0){
          dmax = max(dmax, arr[row_increment][col_increment]);
  ...
  int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3) {
    ...
    int detect_score = diag_max(taxi_matrix, i_meet, j_meet);
