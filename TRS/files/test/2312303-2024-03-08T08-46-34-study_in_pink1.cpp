...
int firstMeet(int& exp1, int& exp2, int e1) {
    ...
    float u = static_cast<float>(e1);
    float exp01 = static_cast<float>(exp1);
    float exp02 = static_cast<float>(exp2);
...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
    ...
    float hp01 = static_cast<float>(HP1);
    float exp01 = static_cast<float>(EXP1);
    ...
    {
        float lower = exp01 - square2 * square2;
        float upper = (square2 + 1) * (square2 + 1) - exp01;
        if (lower < upper)
        {
            S2 = square2 * square2;
        }
...
int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3)
{
    ...
    float exp01 = static_cast<double>(EXP1);
    float exp02 = static_cast<double>(EXP2);
    float hp01 = static_cast<double>(HP1);
    float hp02 = static_cast<double>(HP2);
...
int checkPassword(const char* s, const char* email) {
    ...
    {
        int n = static_cast<int>(pass.find(se));
        return  -(300 + n);
    }
    ...
    for (char x : pass)
    {
        if (isdigit(x) != 0 && isalpha(x) != 0 && dacbiet.find(x) == string::npos)
        {
            return x;
        }
    }
