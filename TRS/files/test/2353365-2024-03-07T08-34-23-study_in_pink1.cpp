...
double update_prob(int exp){
	int num = round(sqrt(exp));
    int s = num * num;
    double p = (double(exp) / s + 80) / 123;
    if (exp >= s) return 1;
    else return p;
}
