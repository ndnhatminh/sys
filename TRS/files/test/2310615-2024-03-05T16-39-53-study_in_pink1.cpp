...
int firstMeet(int & EXP1, int & EXP2, int E1) {
    ...
    if (E1 >= 0 && E1 <= 3) {       
        ...
        if (E1 >= 4 && E1 <= 19) EXP2 += ceil((E1 / 4.0) + 19);
            ...
            EXP2 += ceil((E1 / 4.0) + 19);checkEXP(EXP2);
            if (EXP2 > 200) EXP2 += ceil((E1 / 9.0) + 21);
        }
        else {
            EXP2 += ceil((E1 / 4.0) + 19);checkEXP(EXP2);
            EXP2 += ceil((E1 / 9.0) + 21);checkEXP(EXP2);
            if (EXP2 > 400) {
                EXP2 += ceil((E1 / 16.0) + 17);checkEXP(EXP2);
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    Ptb=(P1+P2+P3)/3.0;
    if (Ptb == 100) EXP1 = ceil(EXP1*75.0/100);
        else {
            if (Ptb < 50) {
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    if (abs(sherlockMax) >= abs(taxi[taxiMax][taxiMin])) {
        EXP1 = ceil(EXP1*112.0/100);checkEXP(EXP1);
        EXP2 = ceil(EXP2*112.0/100);checkEXP(EXP2);
        HP1 = ceil(HP1*11.0/10);checkHP(HP1);
        HP2 = ceil(HP2*11.0/10);checkHP(HP2);
        return abs(sherlockMax);
    }   
    else {
        EXP1 = ceil(EXP1*88.0/100);checkEXP(EXP1);
        EXP2 = ceil(EXP2*88.0/100);checkEXP(EXP2);
        HP1 = ceil(HP1*9.0/10);checkHP(HP1);
