...
int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3) {
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 10; j++) {
            if (mapPoints[i][j] > (E3 * 2)) {
                yang++;
            }
            else if (mapPoints[i][j] < (-E3)) {
                yin++;
            }
        }
    }
    ...
    for (int i = yang, j = yin; i < 10 && j < 10; i++, j++) {
        if (mapPoints[i][j] > sherlock) {
            sherlock = mapPoints[i][j];
        }
    }

    for (int i = yang, j = yin; i < 10 && j >= 0; i++, j--) {
        if (mapPoints[i][j] > sherlock) {
            sherlock = mapPoints[i][j];
        }
    }

    for (int i = yang, j = yin; i >= 0 && j < 10; i--, j++) {
        if (mapPoints[i][j] > sherlock) {
            sherlock = mapPoints[i][j];
        }
    }

    for (int i = yang, j = yin; i >= 0 && j >= 0; i--, j--) {
        if (mapPoints[i][j] > sherlock) {
            sherlock = mapPoints[i][j];
        }
    }
    bool catchup = abs(sherlock) > abs(taxiPoints);  // Kiểm tra nếu người chơi bắt kịp taxi
    if (catchup) {
        HP1 = calculateHP(ceil((float)HP1 * 1.1));  // Tăng HP1 lên 10%
        EXP1 = calculateEXP(ceil((float)EXP1 * 1.12));  // Tăng EXP1 lên 12%
        HP2 = calculateHP(ceil((float)HP2 * 1.1));  // Tăng HP2 lên 10%
        EXP2 = calculateEXP(ceil((float)EXP2 * 1.12));  // Tăng EXP2 lên 12%
    }
    else {
        HP1 = calculateHP(ceil((float)HP1 * 0.9));  // Giảm HP1 đi 10%
        EXP1 = calculateEXP(ceil((float)EXP1 * 0.88));  // Giảm EXP1 đi 12%
        HP2 = calculateHP(ceil((float)HP2 * 0.9));  // Giảm HP2 đi 10%
        EXP2 = calculateEXP(ceil((float)EXP2 * 0.88));  // Giảm EXP2 đi 12%
    }
