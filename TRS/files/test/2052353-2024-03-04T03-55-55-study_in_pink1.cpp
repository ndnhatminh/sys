...
void whereWatsonReturn(int &exp1, int &exp2, int e)
{
    ...
    if (IS_EVEN(oddOrEven))
        exp1 = verifyEXP(exp1 + oddOrEven / 200);
    else
        exp1 = verifyEXP(exp1 - oddOrEven / 100);
}

void brotherOrSister(int &exp1, int &exp2, int e)
{
    if (e >= 4 && e < 20)
        exp2 = verifyEXP(exp2 + FIRST_PIECE(e));
    else if (e >= 20 && e < 50)
        exp2 = verifyEXP(exp2 + SECOND_PIECE(e));
    else if (e >= 50 && e < 66)
        exp2 = verifyEXP(exp2 + THIRD_PIECE(e));
    else if (e >= 65 && e < 80)
    {
        exp2 = verifyEXP(exp2 + FIRST_PIECE(e));
        if (exp2 > 200)
    ...
    {
        exp2 = verifyEXP(verifyEXP(exp2 + FIRST_PIECE(e)) + SECOND_PIECE(e));
        if (exp2 > 400)
            exp2 = verifyEXP(1.15 * verifyEXP(exp2 + THIRD_PIECE(e)));
    }
...
float firstRoute(int &exp)
{
    ...
    if (exp >= closestSquareNumber(exp))
        ...
        firstRouteProb = (float)((exp / closestSquareNumber(exp)) + 80) / 123;
