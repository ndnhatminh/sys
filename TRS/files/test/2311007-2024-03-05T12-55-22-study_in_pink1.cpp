...
int taxi[ROWS][COLS] = { 0 }; // Khởi tạo ma trận 10x10 với giá trị 0

for (int i = 0; i < ROWS; ++i) {
    for (int j = 0; j < COLS; ++j) {
        // Tính điểm số kỹ năng của taxi tại vị trí (i, j)
        int skill = ((E3 * j) + (i * 2)) * (i - j);
        taxi[i][j] = skill;
    }
}
// In ra ma trận điểm số kỹ năng
...
for (int a = i - 1, b = j - 1; a >= 0 && b >= 0; --a, --b) { // Bước 2
    if (taxi[a][b] > max_score) { // Bước 3
        max_score = taxi[a][b];
    }
}

// Hướng xanh dương
for (int a = i + 1, b = j - 1; a < 10 && b >= 0; ++a, --b) { // Bước 2
    if (taxi[a][b] > max_score) { // Bước 3
        max_score = taxi[a][b];
    }
}
...
// Hướng hồng
for (int a = i + 1, b = j + 1; a < 10 && b < 10; ++a, ++b) { // Bước 2
    if (taxi[a][b] > max_score) { // Bước 3
        max_score = taxi[a][b];
    }
}
...
// Hướng vàng
for (int a = i - 1, b = j + 1; a >= 0 && b < 10; --a, ++b) { // Bước 2
    if (taxi[a][b] > max_score) { // Bước 3
        max_score = taxi[a][b];
    }
}
