...
int firstMeet(int &EXP1, int &EXP2, int E1) {
    ...
    if (E1 < 0 || E1 > 99) return -99;
        ...
        if (E1 <= 19) EXP2+= ceil(E1/4.0 + 19);         //Thong tin 1
        else if (E1 <= 49) EXP2+= ceil(E1/9.0 + 21);    //Thong tin 2
        else if (E1 <= 65) EXP2+= ceil(E1/16.0 + 17);   //Thong tin 3
        else if (E1 <= 79) {
            EXP2+= ceil(E1/4.0 + 19);
...
int traceLuggage(int &HP1, int &EXP1, int &M1, int E2) {
    ...
    //Trace 2
        int expense = 0; const double MONEY = M1/2.0; 
        if (E2%2 == 0) for(int i = 0; i < 1; i++)
            { 
                ...
                EXP1= ceil(EXP1*1.13); setEXP(EXP1); setM(M1); 
                if (M1 == 0) break;
                
                //Nguoi dan duong
                if (EXP1 < 300) {M1-= 100; expense+= 100; } else {M1-= 120; expense+= 120; }
                EXP1= ceil(EXP1*0.9); setEXP(EXP1); setM(M1); 
                if (M1 == 0) break;
            }
        
        else while(expense <= MONEY)
        {
            if (HP1 < 200) { HP1= ceil(HP1*1.3); M1-= 150; expense+= 150; } else {HP1= ceil(HP1*1.1); M1-= 70; expense+= 70;}  
            setHP(HP1); setM(M1); 
        ...
        //Di bo
        HP1= ceil(HP1*0.83); EXP1= ceil(EXP1*1.17); setHP(HP1); setEXP(EXP1); 
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    int i, j, ArrTaxi[10][10], ArrSher[10][10], max_lt[10][10], max_rt[10][10];
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) ArrTaxi[i][j] = ((E3*j) + (i*2))*(i - j); 
    }

    int max_l, max_r, i0, j0;
    
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) { 
            //Tim so lon nhat theo cheo trai
            int hieu = i - j;
        
            if (hieu >= 0) {
                i0 = hieu; j0 = 0; max_l = ArrTaxi[i][j];
                for (; i0 < 10; i0++)  {
                    if (ArrTaxi[i0][j0] > max_l) max_l = ArrTaxi[i0][j0];
                    j0++;
                }
            }
        
            else {
                i0 = 0; j0 = -hieu; max_l = ArrTaxi[i0][j0];
                for (; j0 < 10; j0++)  {
                    if (ArrTaxi[i0][j0] > max_l) max_l = ArrTaxi[i0][j0];
                    i0++;
                }
    ...
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) { 
            //Tim so lon nhat theo cheo phai
            int tong = i + j;

            if (tong < 10) {
                i0 = 0; j0 = tong; max_r = ArrTaxi[i0][j0];
                for (; j0 >= 0; j0--)  {
                    if (ArrTaxi[i0][j0] > max_r) max_r = ArrTaxi[i0][j0];
                    i0++;
                }
            }
            
            else {
                j0 = 9; i0 = tong - 9; max_r = ArrTaxi[i0][j0];
                for (; i0 < 10; i0++)  {
                    if (ArrTaxi[i0][j0] > max_r) max_r = ArrTaxi[i0][j0];
                    j0--;
                }
    ...
    //Luu diem cua Sherlock
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) { 
            ArrSher[i][j] = (max_lt[i][j] > max_rt[i][j]) ? abs(max_lt[i][j]) : abs(max_rt[i][j]);
        }
    }
    
    //Xac dinh diem gap nhau
    int i1, j1;
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 10; j++) { 
            if (ArrTaxi[i][j] > 2*E3) i1+= 1;
            
            if (ArrTaxi[i][j] < -E3) j1+= 1;
        }
    }
    while (i1 > 9) {
        i1 = i1/10 + i1%10;
    ...
    const int i_o = i1; const int j_o = j1;
    int res;
    
    //So sanh diem
    if (abs(ArrTaxi[i_o][j_o]) > ArrSher[i_o][j_o]) {
        EXP1= ceil(EXP1*0.88); EXP2= ceil(EXP2*0.88); HP1= ceil(HP1*0.9); HP2= ceil(HP2*0.9);
        res = ArrTaxi[i_o][j_o];
    }
    else {
        EXP1= ceil(EXP1*1.12); EXP2= ceil(EXP2*1.12); HP1= ceil(HP1*1.1); HP2= ceil(HP2*1.1);
        res = ArrSher[i_o][j_o];
    }
    setEXP(EXP1); setEXP(EXP2); setHP(HP1); setHP(HP2);
