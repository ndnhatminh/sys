...
int checkPassword(const char* s, const char* email) {
    ...
    int j;
    if (strlen(s) >= 8 and strlen(s) <= 10 and checkContain == 0
        and checkRepeat == 0
        and checkNoSpecialDigits == 0) checkpass = -10;
    else if (strlen(s) < 8) checkpass = -1;
    else if (strlen(s) > 20) checkpass = -2;
    else if (checkContain(s, se) == true) {
        char* sei;
        sei = strstr(s, se);
        cout << sei - s;
        checkpass = -(300 + (sei - s));
    }
    else if (checkRepeat(s, j) == 1) {
        checkpass = -(400 + j);
    }
    else if (checkNoSpecialDigits(s) == 1) checkpass = -5;
    return checkpass;
}
