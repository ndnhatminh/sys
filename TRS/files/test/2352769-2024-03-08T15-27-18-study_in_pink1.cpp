...
int rangeHP(int &n) {
    if (n<0) {  return n=0;}
    if (n>666) { return n=666;}
    return n;
}
int rangeEXP( int &n) {
    if (n<0) {  return n=0;}
    if (n>600) { return n=600;}
    return n;
}
int rangeM(int &n) {
    if (n<0) { return n=0;}
    if (n>3000) { return n=3000;}
    return n;
}
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    //LAP BAN DO
    int map[10][10],i,j, high=0,low=0;
    int x,y, taxi;
     for(i=0;i<10;i++)
    {
        for(j=0;j<10;j++)
        {
            ...
            if(map[i][j]>(E3*2)) high++;
            else if(map[i][j]<-(E3)) low++;
        }
    }
    //TINH DIEM TAXI
    x=high;
...
while (y>=10)
    ...
    while (sx<9||sy<9) {
            ...
            if (map[sx][sy]>mcc) {mcc=map[sx][sy];}
            sx++;sy++;
    }
    max=mcc;
    sx=x; sy=y;
...
while (sx>=0&&sy>=0&&sx<9&&sy<9) {
    if (map[sx][sy]>max) { max=map[sx][sy];}
    sx-=1; sy+=1;
                                }
sx=x;sy=y;
while (sx>=0&&sy>=0&&sx<9&&sy<9) {
