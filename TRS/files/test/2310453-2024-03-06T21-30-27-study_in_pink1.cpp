...
int firstMeet(int& EXP1, int& EXP2, int E1) {
    ...
    //1.
    if ((E1>=0)&&(E1<=3)){
   if (E1==0){EXP2+=29;rsvl(EXP2,0,600);}
   if (E1==1){EXP2+=45;rsvl(EXP2,0,600);}
   if (E1==2){EXP2+=75;rsvl(EXP2,0,600);}
   if (E1==3){EXP2+=149;rsvl(EXP2,0,600);}
   int D=E1*3+EXP1*7;
   if (D%2==0){EXP1+=ceil(D*1.0/200);} else {EXP1-=ceil(D*1.0/100);} }
   else {
       if ((E1>=4)&&(E1<=19)){EXP2+=19+ceil(E1*1.0/4);rsvl(EXP2,0,600);}
       if ((E1>=20)&&(E1<=49)){EXP2+=21+ceil(E1*1.0/9);rsvl(EXP2,0,600);}
       if ((E1>=50)&&(E1<=65)){EXP2+=17+ceil(E1*1.0/16);rsvl(EXP2,0,600);}
       if ((E1>=66)&&(E1<=79)){EXP2+=19+ceil(E1*1.0/4);rsvl(EXP2,0,600);
           if (EXP2>200){EXP2+=21+ceil(E1*1.0/9);rsvl(EXP2,0,600);}
       }
       if ((E1>=80)&&(E1<=99)){EXP2+=40+ceil(E1*13.0/36);rsvl(EXP2,0,600);
             if (EXP2>400){EXP2+=17+ceil(E1*1.0/16);rsvl(EXP2,0,600);
             EXP2*=1.15; rsvl(EXP2,0,600);
       }
   }
  EXP1-=E1; rsvl(EXP1,0,600); 
}
    return EXP1 + EXP2;
...
if ((E2 < 0) || (E2 > 99)) { return -99; }
   ...
   do {
    ...
    if (E2%2 !=0){
        ...
        if (M1>0){if (EXP1<400){M1=M1-200;} else {M1=M1-120;} EXP1=ceil(EXP1*1.13); rsvl(EXP1,0,600);} else {M1=0;break;}
        if (M1>0){if (EXP1<300){M1=M1-100;} else {M1=M1-120;} EXP1=ceil(EXP1*0.9); rsvl(EXP1,0,600);} else {M1=0;break;}
        }
          HP1=ceil(HP1*0.83); EXP1=ceil(EXP1*1.17);rsvl(EXP1,0,600);
    }
  p2=xs(EXP1);
  ...
  if ((p1+p2+p3)==3){EXP1=ceil(EXP1*0.75);}
   } while ((p1+p2+p3)==3);
  double p=(p1+p2+p3)/3; 
