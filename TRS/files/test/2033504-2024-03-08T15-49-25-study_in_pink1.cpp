...
// Task 3
int calculateIJ(int &i) {
    if (i / 10 >= 1) {
        i = i / 10 + i % 10;
        return calculateIJ(i);
    } else {
        return i;
    }
}
...
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3) {
    ...
    calculateIJ(count_max);
    calculateIJ(count_min);
    int final = abs(matrix[count_max][count_min]);
    int result = matrix[count_max][count_min];
//    cout<<result;
    if (final > EXP1 + EXP2) {
        EXP1 = ceil(EXP1 * 0.88);
        EXP2 = ceil(EXP2 * 0.88);
