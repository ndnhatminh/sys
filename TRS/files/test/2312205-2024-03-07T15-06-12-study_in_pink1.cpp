...
// Macros
#define ceil(double) ceil(float(double)) // Lỡ xài ceil cả bài rồi nên define cho đỡ phải sửa hết =)))
// Function
...
int firstMeet(int &exp1, int &exp2, int e1)
{
    ...
    if (e1 <= 3)
    {
        ...
        D = e1 * 3 + exp1 * 7;
        exp1 = ceil(exp1 + (D % 2 == 0 ? D / 200.0 : -D / 100.0));
    }
    else
    {
        if (e1 <= 19)
...
int traceLuggage(int &HP1, int &EXP1, int &M1, int E2)
{
    ...
    else
    {
        ...
        (E2 <= 9) ? P3 = P[E2] : P3 = P[(E2 / 10 + E2 % 10) % 10];
...
{
    PasswordInfo *pwd = new PasswordInfo[num_pwds];
    ...
    for (int i = 0; i < num_pwds; i++)
    {
        ...
        if (!founded)
        {
            strcpy_s(pwd[num].password, arr_pwds[i]);
            pwd[num].count = 1;
