...
void sherlockFill(int SherlockCar[10][10], int Taxi[10][10])
{
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            ...
            SherlockCar[i][j] = abs(std::max(maxArray(LeftCross, LeftCrossSize), maxArray(RightCross, RightCrossSize)));
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    int SherlockCar[10][10];
    sherlockFill(SherlockCar, Taxi);

    countPos(Taxi, E3); countNeg(Taxi, E3);

    if (abs(Taxi[PositiveNumber][NegativeNumber]) > SherlockCar[PositiveNumber][NegativeNumber])
    {   
    ...
    else
    {
        ...
        return SherlockCar[PositiveNumber][NegativeNumber];
    }
}
...
int findDupeinArray(int* Pos, int OriginalDupePos)
{
    for (int i = 0; i < 30; i++)
    {
        if (Pos[i] == OriginalDupePos) return i;
    }
