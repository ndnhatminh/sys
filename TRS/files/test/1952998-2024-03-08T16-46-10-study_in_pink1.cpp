...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if (P[0] == P[1] && P[1] == P[2] && P[2] == 1.0){
        //mistake, exp1 - 25%
        modifyExp(EXP1, static_cast<double>(EXP1) * -0.25);
    }
    else { //not all is 100%
        //take average P
        if (((P[0] + P[1] + P[2]) / 3) < 0.5){
            modifyExp(EXP1, static_cast<double>(EXP1) * 0.15);
            modifyHP(HP1, static_cast<double>(HP1) * -0.15);
        }
        else { 
            //greater than 50%,quickly find suitcase
            modifyHP(HP1, static_cast<double>(HP1) * -0.1);
            modifyExp(EXP1, static_cast<double>(EXP1) * 0.2);
        }
    }
    return HP1 + EXP1 + M1;
}
