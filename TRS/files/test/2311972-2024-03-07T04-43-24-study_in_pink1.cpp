...
bool checkSepecialChar(int Char){
    if((Char == 33) || (Char == 35) || (Char == 36) || (Char == 37) || (Char == 64)) return 0;
    else if((Char > 47) && (Char < 58)) return 0;
    else if((Char > 64) && (Char < 91)) return 0;
    else if((Char > 96) && (Char < 123)) return 0;
    else return 1;
}
int checkPassword(const char * s, const char * email) {
    ...
    for(int i = 0; i < pass.length(); i++){
        ...
        else if((pass[i]==pass[i+1])&&(pass[i+1]==pass[i+2])){
            sci = i;
            break;
        }
    }
...
int findCorrectPassword(const char* arr_pwds[], int num_pwds) {
    ...
    int A[num_pwds]; //THIS MATRIX CONTAINS THE NUMBER OF STRINGS's OCCURRENCE
    int k = 1;
