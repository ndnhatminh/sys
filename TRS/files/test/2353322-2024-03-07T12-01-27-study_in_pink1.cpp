...
int firstMeet(int & exp1, int & exp2, int e1)
{
    ...
    if (e1 <= 3 && e1 >= 0) {
        ...
        int D = e1 * 3 + exp1 * 7;
        if (D % 2 == 0)
            exp1 = static_cast<int>(std::ceil(exp1 + (D / 200.0) - 0.001)); 
        else
            exp1 = static_cast<int>(std::ceil(exp1 - (D / 100.0) - 0.001));     
    }
...
// Task 3
int Diagonal(int i, int j, int size, int taxiMap[10][10]) {
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3)
{
   if (E3 < 0 || E3 > 99)
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            sherlockWatsonMap[i][j] = Diagonal(i, j, size, taxiMap);
...
int checkPassword(const char * s, const char * email)
{
    ...
    std::string prefix = Email.substr(0, Email.find('@'));
    // length
    if (pass.length() < 8 || pass.length() > 20) {
        return (pass.length() < 8) ? -1 : -2;
    }
    // prefix
    size_t prefixPos = pass.find(prefix);
...
{
  int pos = 0, max_occ = 0, max_len = 0;

    for (int i = 0; i < num_pwds; ++i) {
        int occ = 0, len = std::strlen(arr_pwds[i]);
        ...
        if (occ > max_occ || (occ == max_occ && len > max_len)) {
            max_occ = occ;
            max_len = len;
            pos = i;
        }
