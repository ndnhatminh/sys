...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    int q=M1,n=0;
    if (E2%2==1) {
    while (n<(q*1.0/2) && M1>0)
    {
    if(HP1<200 && M1>0 && n<(q*1.0/2)) {HP1+=ceil(HP1*30.0/100);checkHP(HP1); M1=M1-150;checkM(M1);n=q-M1;}
    if(HP1>=200 && M1>0 && n<(q*1.0/2)) {HP1+=ceil(HP1*1.0/10);checkHP(HP1); M1=M1-70;checkM(M1);n=q-M1;}
    if(EXP1<400 && M1>0 && n<(q*1.0/2)){
        M1-=200; checkM(M1);
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    // TODO: Complete this function
int mtran[10][10],a=0,b=0,A=0,B=0;
if(E3<0 || E3>99) return -99;
for(int i=0; i<10; i++){
        for(int j=0; j<10; j++) {
            mtran[i][j] = ((E3*j)+(i*2))*(i-j);
            if(mtran[i][j]>=E3*2 && mtran[i][j]>0) a++;
            else if(mtran[i][j]<(-E3) && mtran[i][j]<0) b++;
        }
}

A=sum(sum(a));
B=sum(sum(b));
int taxi=mtran[A][B], sumofscore=abs(scoreofSW(mtran,A,B)),output=0;
if(abs(sumofscore)>=abs(taxi)) {
    EXP1+=ceil(EXP1*12.0/100);checkEXP(EXP1);
    EXP2+=ceil(EXP2*12.0/100);checkEXP(EXP2);
    HP1+=ceil(HP1*10.0/100);checkHP(HP1);
    HP2+=ceil(HP2*10.0/100);checkHP(HP2);
    output=sumofscore;
}
else {EXP1=EXP1-floor(EXP1*12.0/100);checkEXP(EXP1);
