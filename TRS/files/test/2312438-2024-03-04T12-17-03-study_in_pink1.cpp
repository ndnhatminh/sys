...
int checkPassword(const char * s, const char * email) {
    ...
    std::stringstream ss;
    ss<<s;
    //Convert const char * to string stream
    std::string s1 =ss.str(); //Conver string stream to string
    std::stringstream ss1;
    ss1<<email;
    std:: string email1 =ss1.str();
    //We get s1, email1 in string type

    int temp = email1.find("@");
    //command find return the position of the first character of the first match
    string se = email1.substr(0,(temp));
    //command substring return a new string that is a substring of this string
    int count = s1.length();
    if (count < 8) return -1;
    if (count > 20) return -2;
    int temp1 = s1.find(se);
    if ( temp1 != -1) return -(300 + temp1);
    for (int i=0; i<(s1.length()-2); i++){
        if ((s1[i+2] == s1[i+1]) && (s1[i+1] == s1[i])) return -(400+i);
    }
    if (s1.find("@")==-1 && s1.find("#")==-1 && s1.find("%")==-1 && s1.find("$")==-1 && s1.find("!")==-1) return -5;
    for (int i=0; i<s1.length(); i++){
        if ( !(s1[i]>='0' && s1[i]<='9') && !(s1[i]>='a'&&s1[i]<='z') && !(s1[i]>='A'&&s1[i]<='Z') && s1[i]!='@' && s1[i]!='#' && s1[i]!='%' && s1[i]!='$' && s1[i]!='!') return i;
    }
    return -10;
}
