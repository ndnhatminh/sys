...
double percentage(int EXP){
    double S;
    double P;
    //tìm S bằng pow(A,x) = A^x  (lũy thừa)
...
// Tách ra nhiều hàm thì bị timelimit exceeded nên để 3 sự kiện gộp lại, thật vô lý :((
void road_2(double & floatHP1, double & floatEXP1, double & floatM1, int & E2,double P2, int money) 
{   // kiểm tra E2 và các điều kiện
...
// Hàm mô phỏng quá trình đi qua 3 tuyến đường
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    //đổi sang các giá trị thập phân để dễ tính toán
    double floatHP1 = HP1;
    double floatEXP1 = EXP1;
    double floatM1 = M1;
    int S;
    ...
    double P2;
    double P3;
    const int money = M1/2;
    //tính p1 của road1
    P1 = percentage(floatEXP1);
    if(chan_le(E2)==true){
        //đi hết một lần nếu E2 chẵn
        ...
        //đi đến khi nào còn nửa cái ví thì ngừng
        while(floatM1 > money){
            road_2(floatHP1,floatEXP1,floatM1,E2,P2,money);
            P2 = percentage(floatEXP1);
            //kiểm tra số tiền
