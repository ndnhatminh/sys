...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    HP1 = ceilf(HP1 - 0.17 * HP1);
    EXP1 = ceilf(EXP1 + 0.17 * EXP1);
    HP1 = checkhp(HP1);
    EXP1 = checkexp(EXP1);
    // Tính xác suất P2
    double P2;
    ...
    // tong ket
    if (P1 == 1 && P2 == 1 && P3 == 1){
        EXP1 = ceilf(EXP1 - 0.25*EXP1);
    }
        else{
            float P = (P1 + P2 + P3)/3;
            if (P < 0.5){
                HP1 = ceilf(HP1 - 0.15*HP1);
                EXP1 = ceilf(EXP1 + 0.15*EXP1);
            }
                else{
                    HP1 = ceilf(HP1 - 0.1*HP1);
                    EXP1 = ceilf(EXP1 + 0.2*EXP1);
                }
        }
    if(EXP1 > 600)
        EXP1 = 600;
