...
void initialmoneyscale(double & M){
    while(M<0||M>3000)
    if(M > 3000){
        M = 3000;
    }
...
void road_2(double & floatHP1, double & floatEXP1, double & floatM1, int & E2,double P2, int initialmoney) {
    if(floatHP1 < 200)
    {
        floatHP1 *= 1.3;
        floatM1 -= 150;
    ...
    initialmoneyscale(floatM1);
    if (chan_le(E2) == true && floatM1 <= 0){
        return;
    }
    else if (chan_le(E2) == false && floatM1 < initialmoney){
        return;
    }
    if(floatEXP1 < 400){ 
        floatM1 -= 200;
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if(chan_le(E2)==true){
        road_2(floatHP1,floatEXP1,floatM1,E2,P2,initialmoney/2);
        if(floatM1<=0){
            floatHP1*=0.83;
            floatEXP1*=1.17;
            expscale(floatEXP1);
        ...
        while(floatM1 >= initialmoney/2){
            road_2(floatHP1,floatEXP1,floatM1,E2,P2,initialmoney/2);
            P2 = percentage(floatEXP1);
            //kiểm tra số tiền
            if(floatM1<initialmoney/2){
                floatHP1*=0.83;
                floatEXP1*=1.17;
                expscale(floatEXP1);
                healthscale(floatHP1);
