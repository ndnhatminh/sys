...
// con duong 2
    if (E2%2 != 0){
        ...
        while (temp <= Mm) {
            if (HP1 < 200){
                HP1 = ceil(130.0/100 * HP1);
                M1 -= 150;
                temp += 150;
                checkEXP(EXP1);checkHP(HP1);
            }
            else{
                HP1 = ceil(110.0/100 * HP1);
                M1 -= 70;
                temp += 70;
                checkEXP(EXP1);checkHP(HP1);
            }
    ...
    if (P1 == 1 && P2 == 1 && P3 == 1) EXP1 = ceil(75.0/100 * EXP1);
        ...
        if (average < 0.5) {
            ...
            EXP1 = ceil(115.0/100 * EXP1);
            checkEXP(EXP1);checkHP(HP1);
        }
        else {
            HP1 = ceil(90.0/100 * HP1);
            EXP1 = ceil(120.0/100 * EXP1);
            checkEXP(EXP1);checkHP(HP1);
        }
    }
...
int checkPassword(const char * s, const char * email){
    ...
    if (ss.size() < 8) return -1;
        ...
        for (int i = 0; i < ss.size(); ++i){
            if(ss[i] < 'a' && ss[i] > 'z' && ss[i] < 'A' && ss[i] > 'Z' && ss[i] < '0' && ss[i] > '9' && ss[i] != '@' && ss[i] != '#' && ss[i] != '%' && ss[i] != '$' && ss[i] != '!'){
                return i;
            }
        }
        return -10;
