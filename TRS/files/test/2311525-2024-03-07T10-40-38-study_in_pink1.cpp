...
int firstMeet(int & exp1, int & exp2, int e1) {
    ...
    if (e1<0 || e1>99) return -99;
        ...
        if (e1>=4 && e1<=19) exp2 += 19+(0.999+e1/4.0);
        if (e1>=20 && e1<=49) exp2 += 21+(0.999+e1/9.0);
        if (e1>=50 && e1<=65) exp2 += 17+(0.999+e1/16.0);
        if (e1>=66 && e1<=79){
            exp2 += 19+(0.999+e1/4.0);
            dieukienEXP(exp2);
            if (exp2>200) exp2 += 21+(0.999+e1/9.0);
        }
        if (e1>=80 && e1<=99){
            exp2 += 19+(0.999+e1/4.0);
            dieukienEXP(exp2);
            exp2 += 21+(0.999+e1/9.0);
            dieukienEXP(exp2);
            if (exp2>400){
                exp2 += 17+(0.999+e1/16.0);
                dieukienEXP(exp2);
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    ...
    string arr_pwds1[30];
    int k=0;
    arr_pwds1[k]=arr_pwds[k];
    for (int i=1;i<num_pwds;i++){
        for (int j=0;j<i;j++){
            if (strcmp(arr_pwds[i],arr_pwds[j])==0) break;
            if (strcmp(arr_pwds[i],arr_pwds[j])!=0 && j==i-1){
                arr_pwds1[k+1]=arr_pwds[i];
                k++;
            }
        }
    }
    int count=0;
    for (count=0;!arr_pwds1[count].empty();count++);
    int arr_pwds2[count];
    for (int i=0;i<count;i++) arr_pwds2[i]=0;
    for (int i=0;i<count;i++){
        for (int j=0;j<num_pwds;j++){
            if (arr_pwds1[i]==arr_pwds[j]) arr_pwds2[i]+=1;
        }
    }
    int max=arr_pwds2[0];
    int pos=0;
    for (int i=0;i<count;i++){
        if (arr_pwds2[i]>max || (arr_pwds2[i]==max && arr_pwds1[i].size()>arr_pwds1[pos].size())){
            max=arr_pwds2[i];
            pos=i;
        }
    }
    for (int i=0;i<num_pwds;i++){
        if (arr_pwds1[pos]==arr_pwds[i]){
            pos=i;
            break;
        }
    }
