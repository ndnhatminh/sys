...
int findCorrectPassword(const char *arr_pwds[], int num_pwds)
{
    ...
    int maxTimesPassLength[lengthiRP] = {};
    ...
    for (int i = 0; i < num_pwds; i++)
    {
        ...
        {
            ++maxTimesPassLength[length];
            ++j;
        }
        ++length;
    }
    // calculateLengthOfMaxTimesPass(arr_pwds, times, iRP, num_pwds, maxTimesPassLength, lengthiRP);
    ...
    for (int i = 0; i < num_pwds; i++)
    {
        if (iRP[i] != 0)
        {
            ...
            if (strlen(arr_pwds[i]) > strlen(arr_pwds[firstRP]))
            {
                return i;
            }
        }
    ...
    /*arr_pwds = {"1234#xyz", "pink#pink", "pink@123", "123!pink", "pink#pink", "pink@123", "pink@123"}
