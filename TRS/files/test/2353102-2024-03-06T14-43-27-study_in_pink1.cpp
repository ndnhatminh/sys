...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2)
{
    ...
    // dng 2
        ...
        if (M1 > 0){
             if (HP1 < 200) {
                HP1 += (int)(HP1 * 0.3 + 0.999); 
                M1 -= 150;
            } else {
                HP1 += (int)(HP1 * 0.1 + 0.999); 
                M1 -= 70;
            }
        }
        if (M1 > 0){
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3)
{
    ...
    //khoi tao ma tran tim check_point
    for (int i = 0; i < 10; i++)
    {
        for(int j = 0; j < 10; j++)
        {
...
{
   int freOfapp[30];
    string psw_ktn[30]; //luu cac phan tu khac nhau
    ...
    //Tim phan tu rieng biet -> psw_ktn
    ...
    //Tim tan suat moi phan tu rieng biet -> tan_suat
    ...
    //Tim tan suat xuat hien nhiu nhat
    int max = freOfapp[0];
    for (int i = 1; i < count; i++)
    {
        if (max < freOfapp[i]) max = freOfapp[i];
    }
    //Tim vi tri cac phan tu co tan suat xuat hien nhiu nhat -> index
    int index[30];
    int count2 = 0;
    int dk = 0;
    for (int i = 0; i < count; i++)
