...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
    ...
    P1 = (EXP1 >= S) ? 1.0 : (EXP1 * 1.0 / S * 1.0 + 80) / 123;
    ...
    while (flag && M1 > m1) {
        ...
        if (HP1 < 200) {
            ...
            HP1 = ceil(float(HP1 * 1.3));    CLAMP_HP(HP1);
            M1 -= 150;                         CLAMP_M(M1);
            aaa = (E2 % 2 == 1) ? 1 : 0;
            ...
            M1 -= 70;                            CLAMP_M(M1);
            HP1 = ceil(float(HP1 * 1.1));    CLAMP_HP(HP1);
            aaa = (E2 % 2 == 1) ? 1 : 0;
        ...
        if (M1 == 0) break;
        CLAMP_EXP(EXP1);
        M1 -= (EXP1 < 400) ? 200 : 120;      CLAMP_M(M1);
        EXP1 = ceil(float(1.13 * EXP1)); CLAMP_EXP(EXP1);
        aaa = (E2 % 2 == 1) ? 1 : 0;
        if (M1 < m1 && aaa) break;

        if (M1 == 0) break;
        M1 -= (EXP1 < 300) ? 100 : 120;     CLAMP_M(M1);
        EXP1 = ceil(float(EXP1 * 0.9)); CLAMP_EXP(EXP1);
        aaa = (E2 % 2 == 1) ? 1 : 0;
        if (M1 < m1 && aaa) break;

        flag = (E2 % 2 == 1) ? 1 : 0;
    }
    HP1 = ceil(float(HP1 * 0.83));     CLAMP_HP(HP1);
    EXP1 = ceil(float(EXP1 * 1.17)); CLAMP_EXP(EXP1);
    S = sqrt(EXP1);
    S = int(abs(EXP1 - pow(S, 2)) < abs(EXP1 - pow(S + 1, 2)) ? pow(S, 2) : pow(S + 1, 2));

    P2 = (EXP1 >= S) ? 1.0 : (EXP1 * 1.0 / S * 1.0 + 80) / 123;
...
int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3) {
    ...
    if (Total > abs(a[x][y]) && E3 != 0) {
        EXP1 = ceil(float(1.12 * EXP1));             CLAMP_EXP(EXP1);
        HP1 = ceil(float(1.10 * HP1));               CLAMP_HP(HP1);
       
        EXP2 = ceil(float(1.12 * EXP2));            CLAMP_EXP(EXP2);
        HP2 = ceil(float(1.10 * HP2));              CLAMP_HP(HP2);
    }
    else {
        EXP1 = ceil(float(0.88 * EXP1));            CLAMP_EXP(EXP1);
        HP1 = ceil(float(0.9 * HP1));               CLAMP_HP(HP1);

        EXP2 = ceil(float(0.88 * EXP2));            CLAMP_EXP(EXP2);
        HP2 = ceil(float(0.9 * HP2));               CLAMP_HP(HP2);
    }
...
int checkPassword(const char* s, const char* email) {
    ...
    int dem = 0;
    int a = 0;
    char* se = tach(const_cast<char*>(email), a);
    ...
    for (int i = 0; i < dem - 1; i++) {
        ...
        if (s[i] == '@' || s[i] == '#' || s[i] == '!' || s[i] == '$' || s[i] == '%') flah = 1;
    }

    if (flah == 0) return -5;
    int flagg = 1;
    for (int i = 0; i < dem && flagg; i++) {
        if ((s[i] >= 'A' && s[i] <= 'Z') || (s[i] >= '0' && s[i] <= '9')) flagg = 1;
        else if (s[i] >= 'a' && s[i] <= 'z') flagg = 1;
        else if (s[i] == '@' || s[i] == '#' || s[i] == '!' || s[i] == '$' || s[i] == '%') flagg = 1;
        else flagg = 0;
        if (flagg == 0) return i;
    }
    delete[] se;
