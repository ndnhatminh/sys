    ...
    if( e1 < 0 || e1 > 99 ) return -99; // Check if the event code is valid
      ...
      if(e1>=0 && e1<=3){                 
        ...
        checkEXP(exp2);
          ...
          if( e1 >= 4 && e1 <= 19 )                 { exp2 += ceil( static_cast<double>(e1)/4 + 19 ); }
            else if(e1>=20 && e1<=49)               { exp2 += ceil( static_cast<double>(e1)/9 + 21 ); }
            else if(e1>=50 && e1<=65)               { exp2 += ceil( static_cast<double>(e1)/16 + 17 );}
            else if(e1>=66 && e1<=79)               { exp2 += ceil( static_cast<double>(e1)/4 + 19);
                 if(exp2 > 200)                       exp2 += ceil( static_cast<double>(e1)/9 + 21 ); }
                 else                               { exp2 += ceil( static_cast<double>(e1)/4 + 19 ); 
                                                      exp2 += ceil( static_cast<double>(e1)/9 + 21 );
                 if(exp2 > 400)                      { exp2 += ceil( static_cast<double>(e1)/16 + 17 );
                                                      exp2 = ceil(static_cast<double>(exp2+exp2*0.15));  }  }
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if( (P1+P2+P3)/3 == 1 ) {
        EXP1 = ceil(static_cast<double>(EXP1-EXP1*0.25)) ;
    }
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
  ...
  // Cập nhật EXP và HP
  if (is_caught) {
    HP1   = ceil(static_cast<double>(HP1+HP1*0.10))     ;
    EXP1  = ceil(static_cast<double>(EXP1+EXP1*0.12))   ;
    HP2   = ceil(static_cast<double>(HP2+HP2*0.10))     ;
...
int checkPassword(const char * s, const char * email) {
    ...
    while( *email != '\0' && *(email+at_ps) != '@' ) { at_ps++ ; }
    ...
    if ( strstr(s, se.c_str()) != NULL ) { return -(300 + strstr(s, se.c_str()) - s); }

    // Checking for consecutive triple characters in string 's' and returning a specific error code if found.
    for ( int i = 0; i < s_length - 2; i++ ) { if ( s[i] == s[i + 1] && s[i] == s[i + 2] )    { return -(400 + i); }  }
    ...
    for ( int i = 0; i < s_length; i++ ) {
        ...
        else if (  s[i] == '@' || s[i] == '#' || s[i] == '%' || s[i] == '$' || s[i] == '!' ) {  has_special_char = true;  break ; }
    ...
    for(int idx = 0; idx < s_length ; idx++ ) {
        if(isalnum(s[idx]) || isalpha(s[idx]) ||   s[idx] == '@' || s[idx] == '#' || s[idx] == '%' || s[idx] == '$' || s[idx] == '!') { continue ; }
        else { return idx ; }
    }
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    ...
    for(int i = 0; i < num_pwds; i++ ) {
        for (int j = 0; j < num_pwds; j++) { if (strcmp(*(arr_pwds+i), *(arr_pwds+j)) == 0) { count_pwds[i]++; } }
    }
    ...
    for (int i = 0; i < num_pwds; i++) {
        if (count_pwds[i] == max_count && strlen(arr_pwds[i]) > strlen(arr_pwds[max_idx])) { max_idx = i ; }
    }
