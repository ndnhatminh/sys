...
bool readFile(
    ...
    double Chinhphuong(int & EXP1){
            double LamTronLen = ceil(sqrt(EXP1));
            double LamTronXuong = floor(sqrt(EXP1));
            double LamTronGanNhat = ((abs(LamTronLen*LamTronLen - EXP1) < abs(LamTronXuong*LamTronXuong - EXP1)) ? abs(LamTronLen*LamTronLen) : abs(LamTronXuong*LamTronXuong));
        return (abs(LamTronGanNhat));
    }
    int traceLuggage(int & HP1, int & EXP1 , int & M1, int E2) {
        if (E2<0 || E2>99) {return -99;} // khi e2 vượt giới hạn

        KiemTraEXP(EXP1); KiemTraHP(HP1); KiemTraTienM(M1);
        double S, P1;
        S = Chinhphuong(EXP1);
        //Road1
        if (EXP1 >= S) {
        ...
        //Road2
        int UsedMoney = 0, TienGoc = M1;
        if(E2%2==1){
            while (float(UsedMoney) <= (0.5*float(TienGoc))){
            //Đi ăn uống
                if (HP1 < 200) {
                    HP1 = (HP1 + ((float)HP1*0.3)) + 0.999; KiemTraHP(HP1);
                    M1 = M1 - 150 ; KiemTraTienM(M1);
                    UsedMoney = UsedMoney + 150;
                    if(float(UsedMoney) > (0.5*float(TienGoc))) break; 

                } else if(HP1 >= 200) {
                    HP1 = (HP1 + ((float)HP1*0.1)) + 0.999; KiemTraHP(HP1);
                    M1 = M1 - 70 ; KiemTraTienM(M1); 
                    UsedMoney = UsedMoney + 70;
                    if(float(UsedMoney) > (0.5*float(TienGoc))) break; 
                }
            //Đi taxi hay ngựa
                if(EXP1 < 400){
                    M1 = M1 - 200; KiemTraTienM(M1);
                    UsedMoney = UsedMoney + 200;
                } else if (EXP1 >= 400) {
                    M1 = M1 - 120; KiemTraTienM(M1);
                    UsedMoney = UsedMoney + 120;
                } 
                EXP1 = (EXP1 + float(EXP1)*0.13)+0.999; KiemTraHP(EXP1);
                if(float(UsedMoney) > (0.5*float(TienGoc))) break;
            //Gặp người vô gia cư
                if(EXP1 < 300){
                    M1 = M1 - 100; KiemTraTienM(M1);
                    UsedMoney = UsedMoney + 100;
                } else if (EXP1 >= 300) {
                    M1 = M1 - 120; KiemTraTienM(M1);
                    UsedMoney = UsedMoney + 100;
                }
                EXP1 = (EXP1 - ((float)EXP1*0.1)) + 0.999; KiemTraEXP(EXP1);
                if(float(UsedMoney) > (0.5*float(TienGoc))) break;
            } 
            HP1 = (HP1 - ((float)HP1*0.17)) + 0.999; KiemTraHP(HP1);
            EXP1 = (EXP1 + ((float)EXP1*0.17)) + 0.999; KiemTraEXP(EXP1);
        }
        if(E2%2==0){
            for(int i=1; i <= 1; i++){
            //Đi ăn uống
            ...
            //Đi taxi hay ngựa
                ...
                EXP1 = (EXP1 + ((float)EXP1*0.13)) + 0.999; KiemTraEXP(EXP1);
        ...
        if (P1==100 && P2==100 && P3==100) {
            EXP1 = (EXP1 - (float(EXP1)*0.25)) + 0.999; KiemTraEXP(EXP1);
        } if(p < 50) {
            HP1 = (HP1 - (float(HP1)*0.15)) + 0.999; KiemTraHP(HP1);
            EXP1 = (EXP1 + (float(EXP1)*0.15)) +0.999; KiemTraEXP(EXP1);
        } else if (p >= 50 && p < 100){
            HP1 = (HP1 - (float(HP1)*0.10)) + 0.999; KiemTraHP(HP1);
            EXP1 = (EXP1 + (float(EXP1)*0.20)) + 0.999; KiemTraEXP(EXP1);
        }
        return HP1 + EXP1 + M1;
    }
// Task 3
int sumOfTwoNumber(int num) {
    ...
    a = a + num/10 + num%10;
    int b=0;
    b = b + a/10 + a%10; //Thêm trường hợp số a cộng lại vẫn ra số có hai chữ số, ví dụ: num= 39, a = 12, b = 3
    return b;
}
int TimSoLonNhatTrenDuongCheo (int &x, int &y,int arr[10][10]){
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    int PositiveNum=0, NegativeNum=0, x,y, DiemXeTaxiLa, DiemSherlockCo;
    ...
    DiemXeTaxiLa = arr[x][y];
    DiemSherlockCo = TimSoLonNhatTrenDuongCheo(x,y,arr);
    if(abs(DiemXeTaxiLa) > abs(DiemSherlockCo)){
...
bool DoTimKyTuTrongChuoi(const char* str) {
    const char KyTuDacBiet[] = "@#%$!";
    for (int i = 0; i < LengthString(str); ++i) {
        for (int j = 0; j < LengthString(KyTuDacBiet); ++j) {
            if (str[i] == KyTuDacBiet[j]) {
                return true;
            }
        }
    }
