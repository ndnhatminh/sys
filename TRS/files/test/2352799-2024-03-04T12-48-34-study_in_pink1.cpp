...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
    ...
    // CON DUONG 02
    float M0 = (float)M1 / 2; // mot nua so tien bat dau con duong
    if (M1 > 0) {
        if (E2 % 2 == 1) {// so le
...
int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3) {
    ...
    while (x > 0 && y < 10) {// duong cheo phai
...
int findCorrectPassword(const char* arr_pwds[], int num_pwds) {
    ...
    for (int i = 0; i < num_pwds; ++i) {// xet do lap + do dai pass
        ...
        if (countpass > maxcount || (countpass == maxcount && length > maxlength)) { // update pass dung 
            maxcount = countpass;
            maxlength = length;
