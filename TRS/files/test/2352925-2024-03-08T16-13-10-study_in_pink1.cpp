...
// Task 2
int check(int value, int minVal, int maxVal) {
...
int traceLuggage(int& HP1, int& EXP1, int& M1, int E2) {
    ...
    while (M1 > 0) {
        // Events in Road 02
        M1 = check(HP1 < 200 ? M1 - 150 : M1 - 70, 0, 3000);
        HP1 = check(ceil(HP1 < 200 ? HP1 * 130 / 100.0 : HP1 * 110 / 100.0), 0, 666);
        if ((E2HD && (M1 < ceil(BDM / 2.0) || !M1)) || (!E2HD && !M1)) break;

        M1 = check(EXP1 < 400 ? M1 - 200 : M1 - 120, 0, 3000);
        EXP1 = check(ceil(EXP1 * 113 / 100.0), 0, 600);
        if ((E2HD && (M1 < ceil(BDM / 2.0) || !M1)) || (!E2HD && !M1)) break;

        M1 = check(EXP1 < 300 ? M1 - 100 : M1 - 120, 0, 3000);
        EXP1 = check(ceil(EXP1 * 90 / 100.0), 0, 600);
        if ((E2HD && (M1 < ceil(BDM / 2.0) || !M1)) || (!E2HD && !M1)) break;
    }

    HP1 = check(ceil(HP1 * 83 / 100.0), 0, 666);
    EXP1 = check(ceil(EXP1 * 117 / 100.0), 0, 600);
...
// Task 3
int maxValue(int matrix[10][10], int i, int j) {
    int maxVal = 0;
    int maxLeft[10 * 2 - 1];  
    int maxRight[10 * 2 - 1]; 
    ...
    for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 10; ++j) {
            int sum = i + j;
            maxLeft[sum] = max(maxLeft[sum], matrix[i][j]);
            maxRight[sum + 10 - 1 - i] = max(maxRight[sum + 10 - 1 - i], matrix[i][j]);
        }
    }

    // Tìm max giữa đường chéo trái và phải tại vị trí (i, j)
    int sum = i + j;
    maxVal = max(maxLeft[sum], maxRight[sum + 10 - 1 - i]);
    return maxVal;
}
    ...
    int chaseTaxi(int& HP1, int& EXP1, int& HP2, int& EXP2, int E3) {
        ...
        int SWpoint = maxValue(matrix, x, y);

        bool Higher = abs(taxiScore) > SWpoint;
        double factor = Higher ? 0.88 : 1.12;

        double bonusEXP1 = EXP1 * factor;
        double bonusEXP2 = EXP2 * factor;

        double bonusHP1 = HP1 * (Higher ? 0.9 : 1.1);
        double bonusHP2 = HP2 * (Higher ? 0.9 : 1.1);

        EXP1 = max(0, min(int(ceil(bonusEXP1)), 600));
        EXP2 = max(0, min(int(ceil(bonusEXP2)), 600));

        HP1 = max(0, min(int(ceil(bonusHP1)), 666));
        HP2 = max(0, min(int(ceil(bonusHP2)), 666));

        return abs(taxiScore) > abs(SWpoint) ? taxiScore : SWpoint;
    }
