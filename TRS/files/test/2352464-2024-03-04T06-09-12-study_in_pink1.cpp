...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    int upper = lower + 1;
    int S;
    if (abs(lower * lower - EXP1) <= abs(upper * upper - EXP1)) {
        S = lower * lower;
    } else {
        S = upper * upper;
    }
    ...
    if (EXP1 >= S){
        ...
        P1 = round((static_cast<double>(EXP1) / S + 80) / 123 * 100) / 100;
    } 
   int pay = M1 * 0.5;                 //road 02
    ...
    if (EXP1 >= S){
        ...
        P2 = round((static_cast<double>(EXP1) / S + 80) / 123 * 100) / 100;
    }  
        int i;                         //road 03
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
   ...
   int greatergrade = max(abs(taxipoint[sumi][sumj]), maxpoint);
   if ((taxipoint[sumi][sumj]) < 0){
      greatergrade *= -1;
   }
    return greatergrade;
    return -1;
