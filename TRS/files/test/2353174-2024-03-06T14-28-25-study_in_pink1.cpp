...
void Prod(int a, int &x, int max)
{
    x *= a;
    if (x > max)
        x = max;
}
void Prod(double a, int &x, int max)
{
    x = ceil(x * a);
    if (x > max)
        x = max;
...
int traceLuggage(int &HP1, int &EXP1, int &M1, int E2)
{
    ...
    else
    {
        ...
        {
            Prod(0.85, HP1, maxhp);
            Prod(1.15, EXP1, maxexp);
        }
        else
        {
            Prod(0.9, HP1, maxhp);
            Prod(1.2, EXP1, maxexp);
        }
        return HP1 + EXP1 + M1;
    }
}
...
int chaseTaxi(int &HP1, int &EXP1, int &HP2, int &EXP2, int E3)
{
    ...
    if (taxi > MAX)
    {
        ...
        Prod(0.88, EXP1, maxexp);
        Prod(0.88, EXP2, maxexp);
        Prod(0.9, HP1, maxhp);
        Prod(0.9, HP2, maxhp);
        return a[x][y];
    }
    ...
    {
        Prod(1.12, EXP1, maxexp);
        Prod(1.12, EXP2, maxexp);
        Prod(1.1, HP1, maxhp);
        Prod(1.1, HP2, maxhp);
        return MAX;
    }
