...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    //Con duong thu 2
        ...
        else
        {
         ...
         if ( EXP1 - int(sqrt(EXP1)) * int(sqrt(EXP1)) < ceil(sqrt(EXP1)) * ceil(sqrt(EXP1)) - EXP1)
        {
...
int checkPassword(const char * s, const char * email) {
    ...
    for( int i = 0 ; i < strlen(s) ; i ++)
    {
        if(( (int)s[i] >= 64 and (int)s[i] <= 90) or ( (int)s[i] >= 97 and (int)s[i] <= 122 ) or s[i] == '@' or s[i] == '#' or s[i] == '%' or s[i] == '$' or s[i] == '!')
        {
            kt = 0 ;
        }
        else
