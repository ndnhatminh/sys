...
int findCorrectPassword(const char *arr_pwds[], int num_pwds)
{
    ...
    bool firstIndexMaxTimes = false;
    for (int i = 1; i < num_pwds; i++)
    {

        if ((times[i] >= times[i + 1]) && (times[i] >= maxTimes))
        {
            iRP[i] = i;
        }
    }
    ...
    if (times[0] == maxTimes)
    {
        firstIndexMaxTimes = true;
        maxLength = strlen(arr_pwds[0]);
        finalIndex = 0;
    }

    if (!firstIndexMaxTimes)
    {
        int k = 1;
        do
        {
            maxLength = strlen(arr_pwds[iRP[k]]);
            finalIndex = k;
            ++k;
        } while (iRP[k] != 0);
    }
    ...
    for (int i = 1; i < num_pwds; i++)
    {
        if (iRP[i] != 0)
        {
            if (maxLength < strlen(arr_pwds[iRP[i]]))
            {
                maxLength = strlen(arr_pwds[iRP[i]]);
                finalIndex = i;
            }
        }
    }
