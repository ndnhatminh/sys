...
//---------------TIM GIA TRI LON NHAT TREN HAI DUONG CHEO----------------
int Diagonal(int TaxiEXP[][10], int row, int column)
{
    ...
    do{
		...
		// Thay doi toa do diem neu hien tai toa do da vuot qua bang 10x10
        if ((j > 9) || (i > 9)){
            if (diff > 0){
                i -= j;
                j = 0;
...
bool Chase(int E3, int &Point)
{
    ...
    int TaxiEXP [10][10] = { };		//Thiet lap bang 10x10
    ...
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            TaxiEXP[i][j] = ((E3 * j) + (i * 2)) * (i - j);			//Thiet lap gia tri diem tai cac toa do (i, j) tuong ung
            if (TaxiEXP[i][j] > E3 * 2)
            {
                ++Pos_count;
            }
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    //-------------Thay doi chi so cua Sherlock va Watson------------
