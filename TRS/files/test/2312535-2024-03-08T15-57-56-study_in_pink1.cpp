...
//////////////////////////////////////////////////////////////////////////////////// 4.5
// Hàm tìm mật khẩu đúng trong mảng arr_pwds
int findCorrectPassword(const char *arr_pwds[], int num_pwds) {
    int maxCount = 0; // Số lần xuất hiện nhiều nhất của một mật khẩu
    int maxLength = 0; // Độ dài lớn nhất của các mật khẩu xuất hiện nhiều nhất
    int index = -1; // Vị trí đầu tiên của mật khẩu đúng
    ...
    for (int i = 0; i < num_pwds; ++i) {
        ...
        int currentCount = 1; // Khởi tạo số lần xuất hiện của mật khẩu hiện tại là 1
        for (int j = i + 1; j < num_pwds; ++j) {
            if (strcmp(arr_pwds[i], arr_pwds[j]) == 0) {
                currentCount++; // Tăng số lần xuất hiện nếu gặp mật khẩu giống nhau
            }
        }
