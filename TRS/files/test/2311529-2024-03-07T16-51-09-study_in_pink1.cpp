...
int checkPassword(const char * s, const char * email) {
    // TODO: Complete this function
    std::size_t emailLength = std::strlen(email);
...
int findCorrectPassword(const char * arr_pwds[], int num_pwds) {
    // TODO: Complete this function
    int correctPasswordIndex = -1;  // Sử dụng -1 để biểu thị không tìm thấy
    int maxCount = 0;
    int maxLength = 0;
    ...
    for (int i = 0; i < num_pwds; ++i) {
        ...
        int currentCount = 0;
        int currentLength = std::strlen(currentPassword);
        ...
        // Kiểm tra xem mật khẩu hiện tại có phải là đúng không
